...
def flush_persistence_sync():
    """حفظ all_users + group_logs + bot_stats في ملف Excel مرة كل 5 دقائق"""
    global ALL_USERS, df_group_logs
    ...
    try:
        ...
        # --- group_logs ---
        df_groups = df_group_logs.copy() if df_group_logs is not None else pd.DataFrame(
        ...
        logging.info("[FLUSH] تم حفظ البيانات في Excel بدون تعطيل البوت.")

    except Exception as e:
        logging.error(f"[FLUSH ERROR] خطأ أثناء حفظ البيانات الدورية: {e}")


async def flush_persistence_job(context: ContextTypes.DEFAULT_TYPE):
    """الغلاف async للجوب"""
    try:
        flush_persistence_sync()
    except Exception as e:
        logging.error(f"[FLUSH JOB] خطأ أثناء حفظ Excel في الجوب: {e}")
...
# ================================================================
#  ⚙️ المرحلة 1 — تخفيف الضغط: إلغاء الكتابة الفورية على Excel
#  الآن كل شيء يُعدَّل في الذاكرة فقط، والحفظ يكون بجوب لاحق
# ================================================================

# 📌 تحديث group_logs (ذاكرة فقط — بدون كتابة للملف)
async def update_group_logs_async(chat):
    ...
    try:
        ...
        logging.info(f"[GROUP_LOGS] buffered update for {title} ({chat_id})")

    except Exception as e:
        logging.error(f"[GROUP_LOGS] ❌ فشل تحديث group_logs في الذاكرة: {e}")


# 📌 حفظ ALL_USERS — (ذاكرة فقط، بدون Excel)
def _update_all_users_log_sync():
    global ALL_USERS
    try:
        # لم نعد نكتب هنا — الحفظ سيكون لاحقًا في جوب واحد
        logging.info(f"[SAVE USERS] buffered {len(ALL_USERS)} users in memory")
    except Exception as e:
        logging.error(f"[SAVE USERS] فشل حفظ all_users_log في الذاكرة: {e}")
...
async def update_all_users_log_async():
    try:
        ...
        logging.error(f"[SAVE USERS] خطأ في تحديث all_users_log في الذاكرة: {e}")


# 📌 عداد استخدام GO — (ذاكرة فقط)
GLOBAL_GO_COUNTER = 0
...
def _update_go_stats_sync():
    ...
    try:
        ...
        logging.error(f"[GO STATS] فشل تعديل العداد في الذاكرة: {e}")
...
async def update_go_stats_async():
    try:
        ...
        logging.error(f"[GO STATS] فشل تحديث العداد في الذاكرة: {e}")
...
async def on_startup():
    ...
    # ✅ تفعيل JobQueue (تنظيف الجلسات القديمة + حفظ الإحصائيات)
    if application.job_queue:
        application.job_queue.run_repeating(cleanup_old_sessions, interval=60 * 60)

        application.job_queue.run_repeating(
            flush_persistence_job,
            interval=300,   # كل 5 دقائق
            first=30        # أول تشغيل بعد 30 ثانية من الإقلاع
        )
