...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type != "private":
        ...
        # ملاحظة: context.user_data هنا هو قاموس هذا المستخدم مباشرة (وليس dict keyed by user_id)
        context.user_data["group_title"] = getattr(chat, "title", None) or "غير معروف"
        context.user_data["group_id"] = chat.id
        context.user_data["final_group_name"] = context.user_data["group_title"]
        context.user_data["final_group_id"] = context.user_data["group_id"]
        ...
        keys_to_remove = [k for k in list(context.user_data.keys()) if str(k).startswith("image_opened_")]
        ...
        try:
            asyncio.create_task(update_group_logs(chat.id, getattr(chat, "title", "") or "", context))
        except Exception as e:
            logging.warning(f"[GROUP_LOGS] فشل جدولة تحديث group_logs للقروب {chat.id}: {e}")

        logging.info(f"[GO من المجموعة] سجلنا بيانات المجموعة {getattr(chat,'title',None)} / {chat.id} للمستخدم {user_name}")
        return
