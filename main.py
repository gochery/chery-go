...
async def handle_recommendation_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إرسال المعاينة بنفس الوسيط إن وجد (آخر وسيط في هذه الرسالة فقط)
    if media_item:
        mtype = media_item["type"]
        fid = media_item["file_id"]
        if mtype == "photo":
...
async def broadcast_recommendation(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if media_list:
                ...
                if non_voice_media:
                    ...
                    for idx, m in enumerate(non_voice_media):
                        ...
                        caption = html_text if idx == 0 else None
                        if mtype == "photo":
                            media_group.append(
                                InputMediaPhoto(
                                    media=fid,
    ...
    # نستخدم فقط أول وسيط غير صوتي لمعاينة المشرفين (وإلا أول voice إن وجد)
    notify_media = None
    non_voice_media = [m for m in media_list if m.get("type") != "voice"]
    voice_media = [m for m in media_list if m.get("type") == "voice"]
    if non_voice_media:
    ...
    for aid in AUTHORIZED_USERS:
        try:
            if notify_media:
                mtype = notify_media["type"]
                fid = notify_media["file_id"]
                if mtype == "photo":
