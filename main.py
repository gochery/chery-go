...
async def show_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if total_ratings_display > 0:
            ...
            rating_info = (
                "⭐ التقييمات:\n"
                f"عدد المقيمين: <code>{total_ratings_display}</code>\n"
                f"متوسط التقييم: <code>{combined_avg}</code> {stars}"
            )
    except Exception:
        pass
    ...
    text = (
        ...
        f"للمستخدم: <code>{user_name_safe}</code>\n\n"
        "<b>📊 ملخص عام:</b>\n"
        f"👥 إجمالي المستخدمين: <code>{total_users}</code>\n"
        f"🏡 عدد القروبات المستخدمة للنظام: <code>{total_groups}</code>\n"
        f"🚀 إجمالي استخدام /go: <code>{total_go}</code>\n\n"
        f"{rating_info}\n\n"
