...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in suggestion_records:
        ...
        user_data["ticket_counter"] = 1  # بدء الترقيم من 1
    else:
        user_data["ticket_counter"] = user_data.get("ticket_counter", 0) + 1
    ...
    # ✅ اختصار suggestion_id لعدم تجاوز حد تيليجرام في callback_data
    short_id = suggestion_id[:8]
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📝 الرد على الاستفسار الوارد", callback_data=f"reply_{user_id}_{short_id}")]
    ])
...
async def handle_send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # البحث عن UUID كامل يبدأ بالـ short_id
    full_uuid = None
    for sug_id in suggestion_records.get(user_id, {}):
        if sug_id.startswith(short_id):
            full_uuid = sug_id
            break
