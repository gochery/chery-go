...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ زر العودة للقائمة بعد التقييم
    if raw_data.startswith("return_"):
        user_id = int(raw_data.split("_")[1])
        await start(update, context)
        await query.answer()
        return
...
async def save_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        keyboard = [
            [InlineKeyboardButton("⬅️ العودة إلى قائمة الخدمات", callback_data=f"return_{user_id}")]
        ]
