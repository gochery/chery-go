...
# 🌟 قيم تعويض الإحصائيات القديمة (سنتين عمل)
BASE_STATS = {
    "users": 3105,
    "groups": 12,
    "go_uses": 9732,
...
async def show_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    صفحة إحصائيات GO + فتح التقييم في نفس الشاشة
    """
    ...
    real_go = 0
    try:
        stats_df = excel_all.get("bot_stats", pd.DataFrame())
        if not stats_df.empty and "key" in stats_df.columns:
            mask = stats_df["key"] == "total_go_uses"
            if mask.any():
                real_go = int(stats_df.loc[mask, "value"].iloc[0])
    except Exception:
        pass
    ...
    try:
        ...
        if not df_ratings.empty and "rating" in df_ratings.columns:
            ...
            rating_info = (
                "⭐ *التقييمات:*\n"
                f"`عدد المقيمين:` {total_ratings}\n"
                f"`متوسط التقييم:` {avg_rating} {stars}"
            )
    except Exception:
        pass

    # === تنسيق مرئي مثل الترحيب ===
    now_saudi = datetime.now(timezone.utc) + timedelta(hours=3)
    refresh_time = (now_saudi + timedelta(minutes=12)).strftime("%I:%M %p")

    text = (
        f"`🧑‍💼 تقرير إحصائيات نظام GO`\n"
        f"`للمستخدم:` {user_name_safe}\n\n"
        "**📊 ملخص عام:**\n"
        f"👥 *إجمالي المستخدمين:* `{total_users}`\n"
        f"🏡 *عدد القروبات المستخدمة للنظام:* `{total_groups}`\n"
        f"🚀 *إجمالي استخدام /go:* `{total_go}`\n\n"
        f"{rating_info}\n\n"
        f"`⏳ يتم تحديث هذه الأرقام تلقائيًا مع نشاط المستخدمين "
        f"({refresh_time} / 🇸🇦)`\n\n"
        "🔹 *فريق GO يشكرك على الثقة والدعم المستمر.*"
    )

    # 👇 هنا الإحصائيات + أيقونات التقييم في نفس الشاشة
...
async def update_group_logs_async(chat):
    """
    🧾 تحديث شيت group_logs بأي قروب يشتغل فيه GO
    يعتمد على الأعمدة:
    - chat_id
    - title
    - type
    ...
    try:
        # نهتم فقط بالقروبات / السوبرقروبات / القنوات
        ...
        # تأكد من وجود الداتا فريم والأعمدة
        if df_group_logs is None:
            df_group_logs = pd.DataFrame(columns=["chat_id", "title", "type", "last_seen_utc"])
        ...
        logging.info(f"[GROUP_LOGS] ✅ تم تحديث group_logs للقروب {title} ({chat_id})")

    except Exception as e:
        logging.error(f"[GROUP_LOGS] ❌ فشل تحديث group_logs للقروب {getattr(chat, 'id', '?')}: {e}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id < 0:
        ...
        return  # ← هذا return يُنهي فرع المجموعة فقط ولا يكسر الدالة

    # ------------------------------------------------------------------------
    # ⬇️ من هنا يبدأ مناداة start() للخاص — وهذا داخل نفس الدالة 100%
    # ------------------------------------------------------------------------
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🧾 لو جت من مجموعة: نحفظ بيانات القروب + نسجلها في group_logs + نفتح جلسة صالحة
    if chat.type != "private":
        ...
        # 🧾 تسجيل / تحديث هذا القروب في group_logs (مهم للإرسال الجماعي والتوصيات)
        try:
    ...
    # ✅ تابع تنفيذ start (ترحيب الخاص + القائمة الرئيسية)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        header = (
            ...
            f"<code>🚗 فئة السيارة: {selected_car_safe}</code>\n\n"
        )
        ...
        # ⚡️ الملاحظة الإضافية عن صور القطع عبر قائمة التصنيف
        note_line = (
            "\n\n<i>💡 يمكن عرض صور قطع الغيار بالتفصيل عبر خيار قائمة التصنيف داخل خدمة قطع الغيار.</i>"
        )
...
async def handle_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # وضع بسيط فقط لو حبيت تستخدمه لاحقاً
    context.user_data.setdefault(user_id, {})["rating_mode"] = True

    await query.answer()

    # 👉 هنا نستدعي صفحة الإحصائيات + أزرار التقييم في نفس الرسالة
