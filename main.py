...
async def reset_manual_search_state(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # احتياطاً
        context.application.user_data.get(user_id, {})
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type != "private":
        ...
        # إنشاء جلسة مؤقتة صالحة لمرة واحدة فقط (ذاكرة المستخدم الحقيقي)
        context.application.user_data.setdefault(user_id, {})
        context.application.user_data[user_id]["session_valid"] = True
...
async def start_suggestion_session(user_id, context):
    ...
    user_state = (context.application.user_data.get(user_id, {}) or {})
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "faults":
        ...
        user_store = (context.application.user_data.get(user_id, {}) or {})
...
async def handle_suggestion_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ تصحيح بيانات المجموعة من الذاكرة الدائمة للنظام
    sys_ctx = (context.application.user_data.get(user_id, {}) or {})

    if record.get("group_name") in [None, "", "خاص", "غير معروف"] or record.get("group_id") in [None, user_id, "غير معروف"]:
        record["group_name"] = (
            sys_ctx.get("final_group_name")
            or sys_ctx.get("group_title")
            or record.get("group_name")
            or "غير معروف"
        )

        record["group_id"] = (
            sys_ctx.get("final_group_id")
            or sys_ctx.get("group_id")
...
async def handle_send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not record.get("replied_by"):
        rc = record.get("reply_count", 0)
        record["reply_count"] = int(rc) if str(rc).isdigit() else 0
    ...
    # =========================
    # ✅ تثبيت بيانات المجموعة بشكل صحيح (بدون ما تتحول إلى user_id)
    # =========================
    def _bad(v):
        return v in (None, "", "خاص", "غير معروف")

    def _normalize_gid(v):
        if v is None:
            return None
        if isinstance(v, int):
            return v
    ...
    rec_gn = record.get("group_name")
    rec_gid = _normalize_gid(record.get("group_id"))
    rec_gid_is_bad = _bad(rec_gid) or rec_gid == user_id

    if _bad(rec_gn) or rec_gid_is_bad:
        uctx = context.user_data.get(user_id, {}) or {}

        fixed_name = uctx.get("final_group_name") or uctx.get("group_title")
        fixed_id = _normalize_gid(uctx.get("final_group_id") or uctx.get("group_id"))

        fixed_id_is_bad = _bad(fixed_id) or fixed_id == user_id

        # احتياط: bot_data (إذا التذكرة بدأت من مجموعة ثم انتقل للخاص)
        if (_bad(fixed_name) or fixed_id_is_bad) and user_id in (context.bot_data or {}):
            bctx = (context.bot_data.get(user_id) or {}) if isinstance(context.bot_data, dict) else {}
            b_name = bctx.get("group_title")
            b_id = _normalize_gid(bctx.get("group_id"))

            if not _bad(b_name):
                fixed_name = fixed_name or b_name
            if not (_bad(b_id) or b_id == user_id):
                fixed_id = fixed_id if not fixed_id_is_bad else b_id

        if not _bad(fixed_name):
            record["group_name"] = fixed_name

        # ✅ أهم سطر: لا نثبت group_id لو يساوي user_id
        if not (_bad(fixed_id) or fixed_id == user_id):
            record["group_id"] = fixed_id

    group_name = record.get("group_name") or "غير معروف"
    group_id = record.get("group_id") or "غير معروف"
    # =========================
    ...
    try:
        ...
        # ✅ لو فشل الإرسال: نفك القفل لو كان هو اللي قفّل
        if locked_now:
            try:
                unlock_ticket(record)
            except:
...
async def submit_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _normalize_gid(v):
        # يحول " -100..." أو "-100..." إلى int إذا ممكن
        if v is None:
            return None
        if isinstance(v, int):
            return v
    ...
    # اعتبر group_id = user_id خطأ
    rec_gid_is_bad = _bad(rec_gid) or rec_gid in (user_id,)

    if _bad(rec_gn) or rec_gid_is_bad:
        uctx = context.user_data.get(user_id, {}) or {}
        fixed_name = uctx.get("final_group_name") or uctx.get("group_title")
        fixed_id = uctx.get("final_group_id") or uctx.get("group_id")
        fixed_id = _normalize_gid(fixed_id)
