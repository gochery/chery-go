...
def build_main_menu_keyboard(user_id: int):
    ...
    [InlineKeyboardButton("🛠️ مركز الدعم والاستفسارات", callback_data=f"suggestion_{user_id}")],
...
async def handle_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    الهاندلر الموحد لازرار الرجوع back:target:user_id
    في المرحلة الاولى يدعم الرجوع الى القائمة الرئيسية فقط
    وباقي الاهداف نضيفها في المراحل القادمة
    """
    query = update.callback_query
    data = query.data

    try:
        _, target, user_id_str = data.split(":")
        user_id = int(user_id_str)
    except Exception as e:
        logging.error(f"[BACK] خطأ في تحليل callback_data للرجوع: {data} / {e}")
        await query.answer("⚠️ تعذر معالجة زر الرجوع.", show_alert=True)
        return
    ...
    if target == "main":
        ...
        try:
            await query.message.edit_text(
                ...
                reply_markup=keyboard  # ✅ تمرير الكيبورد مباشرة
            )

        except Exception as e:
            logging.error(f"[BACK] خطأ اثناء الرجوع للقائمة الرئيسية: {e}")
        return

    # اهداف اخرى لرجوع فرعي نضيفها لاحقا
    await query.answer("ℹ️ زر الرجوع لهذا القسم سيتم تفعيله في التحديث القادم.", show_alert=True)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id < 0:
        ...
        program_description = (
            "🚀 *انطلق الآن مع النسخة المطوّرة من نظام GO*… التجربة الأذكى لخدمة ملاك شيري برو وإكسيد.\n\n"
            "⚙️ *خدمات مدمجة*: صيانة دورية • قطع غيار • دليل المالك • مراكز خدمة ومتاجر معتمدة.\n\n"
            "🔧 *الأعطال الشائعة وحلولها*: معلومات موثوقة ومجرّبة تساعدك على فهم المشكلة بخطوات واضحة.\n"
            "🛠️ *مركز الدعم الفني*: تواصل مباشر مع فريق GO الفني للرد على الاستفسارات ومساعدتك .\n\n"
            "✨ تمت إعادة بناء النظام وتحديث خدماته بالكامل ليقدم لك وصولاً أسرع… ونتائج أدق… وتجربة أكثر احترافية."
        )
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "parts":
        keyboard = [
            ...
            [InlineKeyboardButton("🧩 استعلام قطع غيار اخرى", callback_data=f"extparts_{user_id}")],
            [InlineKeyboardButton("⬅️ رجوع للقائمة الرئيسية", callback_data=f"back_main_{user_id}")]
        ]
        ...
        # نفترض أن unique_cars تم تحضيره مسبقاً كما في الكود الأصلي
        keyboard = []
...
# أزرار الرجوع الموحدة من نوع back:target:user_id (تستخدمها handle_back)
