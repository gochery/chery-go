...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ نظام المحاولات المؤقتة للدخول من الخاص بعد go من المجموعة
    if chat.type == "private" and not context.user_data.get(user_id, {}).get("session_valid") and user_id not in AUTHORIZED_USERS:
        ...
        attempts = context.user_data.setdefault(user_id, {}).setdefault("private_tries", 0)

        if attempts < 5:
            context.user_data[user_id]["private_tries"] += 1
        else:
