...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ السماح الفوري للمشرفين
    if user_id in AUTHORIZED_USERS:
        context.user_data.setdefault(user_id, {})["session_valid"] = True

    # 🟩 الحالة الأولى: المستخدم كتب من مجموعة
    if chat.type in ["group", "supergroup"]:
        context.application._user_data[user_id] = {
            "entry": True,
            "attempts": 0,
    ...
    # 🟥 الحالة الثانية: المستخدم في الخاص وغير مشرف
    elif chat.type == "private" and user_id not in AUTHORIZED_USERS:
        session = context.application._user_data.get(user_id, {})
        from_group = session.get("entry", False)
        attempts = session.get("attempts", 0)

        if not from_group:
            await reply_denied(update, user_name, reason="no_group")
            return

        if attempts >= 5:
            await reply_denied(update, user_name, reason="too_many")
            return
