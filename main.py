...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if attempts >= 3:
        await query.answer(
            "انتهت الاستفسارات في هذه الجلسة...",
            show_alert=True
        )
        user_data.pop("action", None)
        return
    ...
    # إرسال الاستفسار لكل مشرف دون مسح أي بيانات سابقة
    for admin_id in AUTHORIZED_USERS:
        try:
            if media:
                ...
                if mtype == "photo":
                    sent = await context.bot.send_photo(admin_id, fid, caption=caption,
                                                        parse_mode=ParseMode.HTML, reply_markup=keyboard)
                elif mtype == "video":
                    sent = await context.bot.send_video(admin_id, fid, caption=caption,
                                                        parse_mode=ParseMode.HTML, reply_markup=keyboard)
                elif mtype == "document":
                    sent = await context.bot.send_document(admin_id, fid, caption=caption,
                                                           parse_mode=ParseMode.HTML, reply_markup=keyboard)
                elif mtype == "voice":
                    sent = await context.bot.send_voice(admin_id, fid, caption=caption,
                                                        parse_mode=ParseMode.HTML, reply_markup=keyboard)
            else:
                caption = header + (f"\n\n📝 <b>الاستفسار الوارد:</b>\n<code>{text}</code>" if text else "")
                sent = await context.bot.send_message(admin_id, text=caption,
                                                      parse_mode=ParseMode.HTML, reply_markup=keyboard)
            ...
            logging.error(f"فشل في إرسال الاستفسار للمشرف {admin_id}: {e}")
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            "📌 سيتم الرد عليك من قبل فريق الدعم الفني."
        ),
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✉️ إرسال استفسار آخر", callback_data=f"suggestion_{user_id}")]])
    )

    # تنظيف بيانات الكتابة فقط
