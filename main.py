...
BASE_RATINGS = {
    "count": 3050,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
async def handle_recommendation_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🧾 ملخص الوسائط
    photos_count = sum(1 for m in existing_media if m.get("type") == "photo")
    videos_count = sum(1 for m in existing_media if m.get("type") == "video")
    docs_count   = sum(1 for m in existing_media if m.get("type") == "document")
    voices_count = sum(1 for m in existing_media if m.get("type") == "voice")
    ...
    text_status = "نص التوصية موجود" if text else "التوصية بدون نص (وسائط فقط)"
    ...
    # ✅ المعاينة الآن نص فقط، مرتبة + فيها ملخص الوسائط
...
async def broadcast_recommendation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ترتيب الأنواع: فيديو ثم صورة ثم ملف
        ...
        try:
            ...
            if media_list:
                try:
                    ...
                    if non_voice_media:
                        # 🔢 ترتيب: فيديو → صور → مستندات
                        non_voice_media = sorted(
                            non_voice_media,
                            key=lambda m: type_order.get(m.get("type"), 3)
                        )

                        # 🎞️ نرسل الفيديوهات والصور والملفات كألبوم واحد *بدون* كابتشن
                        media_group = []
                        ...
                        sent_messages = []
                        if media_group:
                            sent_messages = await context.bot.send_media_group(chat_id, media_group)

                        # بعد الألبوم نرسل التوصية كنص مستقل تحته
                        text_msg = None
                        if html_text:
                        ...
                        # بعدها نرسل أي voice بدون كابتشن
                        for v in voice_media:
                            try:
                                vf = v.get("file_id")
                                if vf:
                        ...
                        # الرسالة اللي بنثبتها (لو طلب التثبيت): نفضّل نص التوصية
                        sent_msg = text_msg or (sent_messages[0] if sent_messages else None)

                    else:
                        # لا يوجد إلا voice فقط → نرسل أول voice مع النص والباقي بدون
