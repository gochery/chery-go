...
PP_DIRECT_ENABLED = False   # ✅ حاليا غير مفعّل
PP_BOT_USERNAME = ""        # لاحقا: اسم البوت بدون @ مثال: "PP_OfficialBot"
...
BASE_RATINGS = {
    "count": 4891,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
def build_main_menu_keyboard(user_id: int) -> InlineKeyboardMarkup:
    keyboard = [
        ...
        [InlineKeyboardButton("🛒 منصة قطع الغيار PP", callback_data=f"coming_{user_id}")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type == "private" and not context.user_data.get(user_id, {}).get("session_valid") and user_id not in AUTHORIZED_USERS:
        ...
        if text in ["/start", "start", "go", "/go"] and "start=go" not in text:
            alert_message = (
               "📣 يسعدنا اهتمامك بخدمات *نظام الصيانة وقطع الغيار GO*!\n\n"
  ...
  # ✅ إذا النداء من مجموعة: نرسل بانر الترحيب ونخرج
    ...
    # ✅ اختصار شاشة الترحيب (اسم + سطرين فقط)
        full_caption = (
            f"`👤 {user_name}`\n"
            "✨ مرحبًا بك في نظام الصيانة والدعم الفني GO ومنصة PPلقطع الغيار\n"
        )
    ...
    mmsg1 = await update.message.reply_text(
        f"`🧑‍💼 مرحباً {user_name}`\n\n"
        "🚀 *أهلًا بك في نظام لخ GO ومنصة PP للاستعلام الفني والخدمات المساندة.*\n"
        "يوفّر لك النظام معلومات دقيقة عن الصيانة، وحلول الأعطال، ودليل الاستخدام، مع منصة PP لشراء قطع الغيار ودعم فني مباشر عند الحاجة.\n\n"
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type == "private" and (
        ...
        user_block = f"🧑‍🏫 مرحبا {user_name}"
        alert_message = (
            "📣 يسعدنا اهتمامك بخدمات *نظام الصيانة GO ومنصة pp لقطع الغيار *!\n\n"
            "❌ لا يمكنك بدء الخدمة مباشرة من الخاص.\n"
            "🔐 حفاظًا على الخصوصية، يرجى العودة إلى مجموعتك أو الانضمام إلى المجموعة الرئيسية أدناه وكتابة الأمر (go) هناك.\n\n"
            "[👥 اضغط هنا للانضمام إلى مجموعة CHERY KSA ](https://t.me/CHERYKSA_group)"
        )
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔹 رسائل القطع داخل التصنيف
        ...
        teaser = (
            ...
            "<b>📋 للبدء نحتاج فقط:</b>\n"
            "• <i>اسم السيارة</i>\n"
            "• <i>الموديل والفئة (سنة من 4 ارقام + الفئة)</i>\n"
            ...
            "</i>\n\n"
            "<b>⬇️ لبدء طلب جديد بالخاص ارسل  كلمة pp فقط</b>"
        )
