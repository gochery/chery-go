...
async def start_recommendation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data[admin_id]["reco_entities"] = None
    context.user_data[admin_id]["reco_selected"] = []
...
async def handle_recommendation_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    preview_caption = (
        ...
        "✅ يمكنك الآن:\n"
        "• بث التوصية على *جميع المجموعات* مباشرة.\n"
        "• أو اختيار *مجموعات محددة يدويًا* قبل البث.\n"
    )

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📡 بث على جميع المجموعات", callback_data="reco_broadcast_all")],
        [InlineKeyboardButton("🧾 اختيار المجموعات يدوياً", callback_data="reco_select")],
...
def build_reco_groups_keyboard(admin_id: int, context: ContextTypes.DEFAULT_TYPE) -> InlineKeyboardMarkup:
    """يبني كيبورد اختيار المجموعات مع ترقيم الصفحات"""
    ud = context.user_data.setdefault(admin_id, {})
    ...
    if not groups:
        rows.append([InlineKeyboardButton("⚠️ لا توجد مجموعات مسجلة للبث حالياً", callback_data="reco_noop")])
...
async def show_reco_groups(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text_lines = [
        ...
        "• يمكنك التنقل بين الصفحات (السابق / التالي).",
...
async def broadcast_recommendation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """بث التوصية على المجموعات (الكل أو المحدد فقط) + إشعار جميع المشرفين"""
    query = update.callback_query
    admin_id = query.from_user.id
    admin_name = query.from_user.full_name
    ...
    # تحديد المجموعات:
    # - إذا جاء من زر "reco_broadcast_all" → كل المجموعات
    # - إذا جاء من "reco_broadcast" ومعه reco_selected → المجموعات المحددة
    # - إذا قائمة reco_selected فارغة → نرجع لسلوك الكل
    data = query.data or ""
        ...
        try:
            ...
            if media:
                ...
                await context.bot.send_message(
                    chat_id,
                    text,
                    entities=entities,              # نفس تنسيق تيليجرام (رابط مخفي)
                    disable_web_page_preview=True,  # منع معاينة الروابط الظاهرة
                )
...
# ✅ اختيار المجموعات يدوياً للتوصية
