...
async def submit_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ⭐⭐ دعم الرد الجاهز: إذا لم يوجد نص في compose_text، انسخ ready_reply_text مباشرةً إليها
    if not reply_text:
        ready_reply = context.user_data[admin_id].get("ready_reply_text")
        if ready_reply:
            context.user_data[admin_id]["compose_text"] = ready_reply  # ✅ ضع النص في compose_text
