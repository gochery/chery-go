...
BASE_RATINGS = {
    "count": 4623,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
     "avg": 4.9,     # 👈 متوسط التقييم (من 5)
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id < 0:
        ...
        program_description = (
            "**🚀 ابدأ الآن تجربتك مع النسخة المطوّرة من نظام GO**\n"
            "`التجربة الأذكى لخدمة ملاك شيري / إكسيد / ام جي / جايكو / أومودا / سوايست / جيتور / بي واي دي.`\n\n"
            "**⚙️ خدمات تفاعلية**\n"
...
async def start_suggestion_session(user_id, context):
    """
    إنشاء جلسة اقتراح جديدة أو إعادة استخدام جلسة موجودة غير مكتملة.
    """
    logging.info(f"[SESSION] بدء التحقق من جلسة المستخدم {user_id}")

    user_state = context.user_data.get(user_id, {})
    active_id = user_state.get("active_suggestion_id")

    # إعادة استخدام جلسة غير مكتملة
    if active_id and user_id in suggestion_records:
        ...
        if existing_record and not existing_record.get("submitted"):
            logging.info(f"[SESSION] إعادة استخدام الجلسة الحالية: {active_id}")
            return active_id
    ...
    # استرداد بيانات المستخدم / المجموعة
    ...
    # fallback من bot_data إذا البيانات غير موجودة
    ...
    logging.info(f"[SESSION] تم إنشاء سجل جديد: {suggestion_id} للمستخدم {user_id}")
    return suggestion_id


async def receive_user_message(user_id, message_text, media, context):
    """
    استلام رسالة من المستخدم، وتعبئة السجل قبل أي رد.
    """
    ...
    if not suggestion_id:
        ...
        logging.warning(f"[RECEIVE] لم يكن هناك جلسة، تم إنشاء جلسة جديدة: {suggestion_id}")

    record = suggestion_records.setdefault(user_id, {}).setdefault(suggestion_id, {})

    # تعبئة نص الرسالة والوسائط
    record["text"] = message_text or "❓ لم يرسل المستخدم نصاً"
    record["media"] = media
    logging.info(f"[RECEIVE] سجل المستخدم {user_id} تم تحديثه: text={record['text']}, media={'نعم' if media else 'لا'}")

    # علامة الإرسال لتجنب إعادة استخدام قديمة
...
async def broadcast_recommendation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    entities = ud.get("reco_entities")  # تنسيقات تيليجرام (لن نستخدمها هنا بعد التحويل لـ HTML)
    pin_enabled = bool(ud.get("reco_pin", False))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode in ["suggestion", "custom_reply"] and chat.type == "private":
...
async def handle_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إذا كان المستخدم داخل عملية (استفسار/ملاحظة)
    if mode == "suggestion":
...
async def handle_suggestion_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if prefix not in ["reply", "replyready", "sendreply", "customreply"]:
        await query.answer("🚫 بيانات غير صالحة.", show_alert=True)
        return
    ...
    # ===== استخراج user_id و suggestion_id =====
    if prefix == "sendreply":
        if len(data_parts) < 4:
            await query.answer("❌ بيانات غير صحيحة للرد الجاهز.", show_alert=True)
            return
        encoded_key = data_parts[1]
        ready_key = urllib.parse.unquote_plus(encoded_key)
    ...
    user_id_str = user_id_str.replace("reply", "")
    try:
        user_id = int(user_id_str)
    except ValueError:
        await query.answer("🚫 بيانات غير صالحة: USER_ID غير صحيح.", show_alert=True)
        return

    user_records = suggestion_records.get(user_id)
    if not user_records:
        await query.answer(f"❌ لا يوجد سجل للمستخدم {user_id}.", show_alert=True)
        return

    record = user_records.get(suggestion_id)
    if not record:
        logging.warning(f"[handle_suggestion_reply] سجل غير موجود: user_id={user_id}, suggestion_id={suggestion_id}")
        await query.answer(
            f"❌ لا يوجد سجل لهذا الاستفسار.\n👤 المستخدم: {user_id}\n🆔 ID التذكرة: {suggestion_id}",
            show_alert=True
        )
        return
    ...
    if prefix == "reply":
        ...
        # replyready أو sendreply: توليد كيبورد الردود الجاهزة
        keyboard = [
            [
                InlineKeyboardButton(
                    text,
                    callback_data=f"sendreply_{urllib.parse.quote_plus(key)}_{user_id}_{suggestion_id}"
                )
            ]
            for key, text in SUGGESTION_REPLIES.items()
        ]
        keyboard.append([InlineKeyboardButton("⬅️ رجوع لاختيار نوع الرد", callback_data=f"reply_{user_id}_{suggestion_id}")])
        ...
        if prefix == "sendreply":
            ready_text = SUGGESTION_REPLIES.get(ready_key, "")
            context.user_data[admin_id]["ready_reply_text"] = ready_text
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if attempts >= 3:
        user_name = query.from_user.full_name  # الحصول على الاسم الحقيقي
        await query.answer(
            f"🚫 انتهت جميع الاستفسارات المقررة في هذه الجلسة، {user_name}.\n\n"
            "⚠️ الرجاء الضغط على زر الإلغاء لإنهاء الجلسة الحالية، ثم العودة إلى المجموعة وكتابة GO مرة أخرى للوصول إلى مركز الدعم.",
            show_alert=True
        )
        user_data.pop("action", None)
        return
    ...
    # رقم التذكرة
    if "ticket_counter" not in user_data:
        user_data["ticket_counter"] = 1
    else:
        user_data["ticket_counter"] += 1

    ticket_no = user_data["ticket_counter"]

    if user_id not in suggestion_records:
        suggestion_records[user_id] = {}

    # استخدام ticket_no كمفتاح بدلاً من UUID طويل
    suggestion_id = str(ticket_no)
    user_data["active_suggestion_id"] = suggestion_id

    suggestion_records[user_id][suggestion_id] = {
        "text": user_data.get("compose_text", ""),
        "media": user_data.get("compose_media"),
        ...
        "reply_count": 0,
        "user_name": query.from_user.full_name,
        "group_name": user_data.get("final_group_name", "غير معروف"),
        "group_id": user_data.get("final_group_id", "غير معروف"),
    }
    ...
    keys_to_clear = ["action", "compose_mode", "compose_text", "compose_media", "active_suggestion_id"]
...
async def handle_send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id_str = data_parts[1].replace("reply", "")
    try:
        user_id = int(user_id_str)
    except ValueError:
        await query.answer("🚫 بيانات غير صالحة: user_id غير صحيح.", show_alert=True)
    ...
    ticket_no = data_parts[2]

    # استخدام ticket_no مباشرة للبحث
    record = suggestion_records.get(user_id, {}).get(ticket_no)
    if not record:
    ...
    ticket_info_user = (
        ...
        f"\u200F🔁 *رقم الرد داخل التذكرة:* `{reply_count}`\n\n"
    )
    ticket_info_admin = ticket_info_user

    if is_additional:
        user_caption = (
            f"\u200F🔁 *رد إضافي رقم {reply_count} من فريق الدعم الفني GO:*\n\n"
            f"{ticket_info_user}"
    ...
    try:
        ...
        if media:
            ...
            fid = media["file_id"]
            if mtype == "photo":
                await context.bot.send_photo(user_id, fid, caption=user_caption, parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=True)
            elif mtype == "video":
                await context.bot.send_video(user_id, fid, caption=user_caption, parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=True)
            elif mtype == "document":
                await context.bot.send_document(user_id, fid, caption=user_caption, parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=True)
            elif mtype == "voice":
            ...
            try:
                with open("GO-NOW.PNG", "rb") as image:
                    await context.bot.send_photo(user_id, image, caption=user_caption, parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=True)
            except Exception:
        ...
        for aid in AUTHORIZED_USERS:
            try:
                buttons = [
                    ...
                    [InlineKeyboardButton("🗣️ دعوة إبداء رأي", callback_data=f"team_opinion_{user_id}_{ticket_no}")],
                ]

                if aid == admin_id:
                    buttons.insert(
                        0,
                        [InlineKeyboardButton("✉️ إرسال رد آخر", callback_data=f"customreply_{user_id}_{ticket_no}")]
                    )
                ...
                if media:
                    ...
                    fid = media["file_id"]
                    if mtype == "photo":
                        await context.bot.send_photo(aid, fid, caption=admin_caption, parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup, disable_web_page_preview=True)
                    elif mtype == "video":
                        await context.bot.send_video(aid, fid, caption=admin_caption, parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup, disable_web_page_preview=True)
                    elif mtype == "document":
                        await context.bot.send_document(aid, fid, caption=admin_caption, parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup, disable_web_page_preview=True)
                    elif mtype == "voice":
                        await context.bot.send_voice(aid, fid, caption=admin_caption, parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup)
                else:
                    try:
                        with open("GO-NOW.PNG", "rb") as image:
                            await context.bot.send_photo(aid, image, caption=admin_caption, parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup, disable_web_page_preview=True)
                    except Exception:
...
async def submit_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔍 كشف تفصيلي قبل أي عملية
    logging.debug(f"[DEBUG SUBMIT_REPLY] admin_id={admin_id}, user_id={user_id}, suggestion_id={suggestion_id}")
    logging.debug(f"[DEBUG SUBMIT_REPLY] context.user_data[admin_id]={context.user_data.get(admin_id)}")
    logging.debug(f"[DEBUG SUBMIT_REPLY] suggestion_records.keys()={list(suggestion_records.keys())}")
    logging.debug(f"[DEBUG SUBMIT_REPLY] user_records={(list(suggestion_records.get(user_id, {}).keys()) if user_id in suggestion_records else 'لا يوجد')}")

    if not user_id or not suggestion_id:
        logging.error(f"[DEBUG SUBMIT_REPLY] لا توجد جلسة رد نشطة. user_id={user_id}, suggestion_id={suggestion_id}")
        await query.answer("❌ لا توجد جلسة رد نشطة.", show_alert=True)
        return

    record = suggestion_records.get(user_id, {}).get(suggestion_id)
    if not record:
        logging.error(f"[DEBUG SUBMIT_REPLY] لا يوجد سجل لهذه الاستفسار! user_id={user_id}, suggestion_id={suggestion_id}")
        await query.answer("❌ لا يوجد سجل لهذه الاستفسار.", show_alert=True)
        return
    ...
    raw_text = context.user_data[admin_id].get("compose_text")
    reply_text = (raw_text or "").strip()

    # ⭐⭐ دعم الرد الجاهز
    if not reply_text:
        ready_reply = context.user_data[admin_id].get("ready_reply_text")
        if ready_reply:
            context.user_data[admin_id]["compose_text"] = ready_reply
            reply_text = ready_reply.strip()
            logging.debug(f"[DEBUG SUBMIT_REPLY] استخدام ready_reply_text: {reply_text}")
    ...
    if not reply_text and not media:
        logging.error(f"[DEBUG SUBMIT_REPLY] الرد فارغ تماماً. user_id={user_id}, suggestion_id={suggestion_id}")
    ...
    # نضيف فقرة "رد المشرف" فقط لو فيه نص فعلي مع الرابط المخفي
    if reply_text:
        user_caption += f"\u200F💬 *رد المشرف:*\n```{reply_text}```{hidden_link}\n\n"

    user_caption += f"\u200F🤖 *شكرًا لثقتك بفريق الصيانة والدعم الفني GO.*"
    ...
    # برضه هنا: نضيف "رد المشرف" فقط لو فيه نص مع الرابط المخفي
    if reply_text:
    ...
    try:
        ...
        for aid in AUTHORIZED_USERS:
            try:
                # كيبورد موحد لكل رد للمشرفين
                buttons = [
                    [
                        InlineKeyboardButton(
                            "🟦 دعوة فريق GO للنقاش",
                            callback_data=f"team_main_{aid}",
                        )
                    ],
                    [
                        InlineKeyboardButton(
                ...
                # زر إرسال رد آخر يظهر فقط للمشرف الذي أرسل الرد
                if aid == admin_id:
                    buttons.insert(
                        0,
                        [
    ...
    if query.data == "list_admins":
        try:
            ...
            try:
                ...
                df_admins_local = df_admins  # نسخة fallback
...
def collect_target_chat_ids(context: ContextTypes.DEFAULT_TYPE) -> set[int]:
    """يجمع chat_id من:
       شيت group_logs (كل المجموعات/القنوات المسجلة)
    """
    targets: set[int] = set()
    global df_group_logs

    # 1) من شيت group_logs فقط
    try:
        if df_group_logs is not None and not df_group_logs.empty and "chat_id" in df_group_logs.columns:
            for val in df_group_logs["chat_id"].dropna().tolist():
                try:
                    ...
                    continue
                # مجموعات/قنوات تيليجرام تكون ID سالب
                if isinstance(cid, int) and cid < 0:
                    targets.add(cid)
    except Exception as e:
        logging.warning(f"[RECO TARGETS] فشل تحميل المجموعات من df_group_logs: {e}")

    logging.info(f"[RECO TARGETS] عدد المجموعات المتاحة للبث (من group_logs فقط): {len(targets)}")
    return targets
