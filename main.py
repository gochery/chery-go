...
async def handle_recommendation_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    preview_caption = (
        ...
        "✅ إذا كانت مناسبة اختر «بث التوصية على جميع المجموعات» أو استخدم «اختيار المجموعات يدويًا» أو أرسل رسالة جديدة لتعديل النص قبل الإرسال."
    )

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📡 بث التوصية على جميع المجموعات", callback_data="reco_broadcast")],
        [InlineKeyboardButton("📋 اختيار المجموعات يدويًا", callback_data="reco_choose_groups")],
...
application.add_handler(CallbackQueryHandler(cancel_recommendation, pattern=r"^(reco_cancel|cancel_reco)$"))
