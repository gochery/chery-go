...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ إعادة تهيئة بيانات الجلسة للمستخدم
    context.user_data.setdefault(user_id, {})
    context.user_data[user_id]["manual_sent"] = False
    context.user_data[user_id]["session_valid"] = True
    context.user_data[user_id]["suggestion_clicks"] = 0   # ← إعادة ضبط عداد ضغطات زر الاستفسار
    ...
    # استرجاع بيانات المجموعة المحفوظة
    ...
    if chat_id < 0:
        ...
        # ✅ بعد إرسال رسالة الترحيب في المجموعة، حدّث group_logs في الخلفية
        try:
...
async def clear_buttons_after_session(user_id, context: ContextTypes.DEFAULT_TYPE):
    """
    🔹 تقوم بحذف أي رسالة تحتوي أزرار مرتبطة بالجلسة المنتهية للمستخدم
    """
    user_msgs = context.user_data.get(user_id, {}).get("registered_messages", [])
    for chat_id, msg_id in user_msgs:
        try:
            await context.bot.edit_message_reply_markup(chat_id=chat_id, message_id=msg_id, reply_markup=None)
        except Exception:
            pass
    # مسح الرسائل المسجلة
    if "registered_messages" in context.user_data.get(user_id, {}):
        context.user_data[user_id]["registered_messages"].clear()
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🧑‍💻 من الخاص
    if chat.type == "private":
        user_session = context.user_data.get(user_id, {}).get("session_valid")

        # إذا الجلسة غير صالحة والمستخدم ليس مشرفًا → لا يسمح بالتفاعل
        if not user_session and user_id not in AUTHORIZED_USERS:
            # رسالة تنبيه + رابط العودة للمجموعة
            msg = await update.message.reply_text(
                f"{user_block}\n\n{alert_message}\n\n{delete_block}",
                parse_mode=constants.ParseMode.MARKDOWN,
                disable_web_page_preview=True
        ...
        # ✅ إذا الجلسة صالحة أو المستخدم مشرف → نسمح باستدعاء start
        await start(update, context)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔒 تجاهل أي رسالة بالخاص إذا لم تكن هناك جلسة صالحة
    if chat.type == "private" and not context.user_data[user_id].get("session_valid", False) and user_id not in AUTHORIZED_USERS:
        return
    ...
    # 🔒 منع ظهور أزرار الإرسال/الإلغاء لأي رسالة نصية ما لم يكن المستخدم فعلياً داخل جلسة suggestion
    if chat.type == "private":
        user_action = context.user_data.get(user_id, {}).get("action")
        if user_action not in ["suggestion"]:
            return
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ تفعيل الجلسة عند الضغط على زر الاستفسار
        context.user_data[user_id]["session_valid"] = True
    
    # ⬅️⬅️ هنا نضع السطر المطلوب
        context.user_data[user_id]["allow_new_question"] = False
        
    # تنظيف أي جلسة استفسار سابقة
        if "active_suggestion_id" in context.user_data[user_id]:
            del context.user_data[user_id]["active_suggestion_id"]
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تهيئة عداد النقرات إذا لم يكن موجود
    context.user_data.setdefault(user_id, {})
    clicks = context.user_data[user_id].get("suggestion_clicks", 0)

    # تحقق إذا تجاوز الحد
    if clicks >= 3:
        await query.answer("⚠️ لقد وصلت للحد الأقصى من إرسال الاستفسارات. استخدم /go لإعادة تفعيل الخدمة.", show_alert=True)
        return
    ...
    # رسالة شكر + رقم التذكرة + اقتباس للاستفسار
    thank_you_message = (
    ...
    # زيادة عداد النقرات
    clicks += 1
    context.user_data[user_id]["suggestion_clicks"] = clicks

    # تحديد ظهور زر "إرسال استفسار آخر" فقط إذا لم يصل الحد
    if clicks < 3:
        back_keyboard = InlineKeyboardMarkup(
            [
        ...
        back_keyboard = None
        # بعد الحد يتم إنهاء الجلسة مؤقتًا
        context.user_data[user_id]["active_suggestion_id"] = None
