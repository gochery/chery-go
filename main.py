...
BASE_RATINGS = {
    "count": 1935,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
# ================================================================
#  تحميل بيانات Excel مع دعم النسخ الاحتياطية
# ================================================================
try:
    # نحاول التحميل مع fallback على آخر باك أب
    excel_data = _load_excel_with_backup()

    # ================================================================
    #  تحميل قائمة المشرفين AUTHORIZED_USERS
    # ================================================================
    try:
        ...
        elif "id" in df_admins.columns:  # حماية إضافية لو العمود اسمه مختلف
            AUTHORIZED_USERS = (
                pd.to_numeric(df_admins["id"], errors="coerce")
                .dropna()
                .astype(int)
                .tolist()
            )
        ...
        logging.info(f"[ADMINS] Loaded {len(AUTHORIZED_USERS)} admins")

    except Exception as e:
        logging.error(f"[ADMINS ERROR] فشل تحميل قائمة المشرفين: {e}")
        AUTHORIZED_USERS = []
    ...
    # ================================================================
    #  استخراج قائمة السيارات الفريدة لقطع الغيار
    # ================================================================
    try:
        unique_cars = sorted(
            df_parts["Station No"]
    ...
    # ================================================================
    #  branches → قائمة من dicts في bot_data (حل مشكلة الفروع)
    # ================================================================
    try:
...
async def show_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        try:
            STATS_CACHE["excel_all"] = pd.read_excel("bot_data.xlsx", sheet_name=None)
        except Exception:
...
async def update_group_logs(chat_id: int, chat_title: str, context: ContextTypes.DEFAULT_TYPE):
    """تسجيل القروبات في شيت group_logs بشكل سريع وآمن"""
    global df_group_logs

    if chat_id >= 0:
        return  # يسجل فقط المجموعات (ID سالب)
