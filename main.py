...
# 🔐 معرف مجموعة النسخ الاحتياطي (اختياري) - يتم ضبطه من متغير البيئة TG_BACKUP_CHAT_ID
BACKUP_CHAT_ID_STR = os.getenv("TG_BACKUP_CHAT_ID")
try:
    TG_BACKUP_CHAT_ID = int(BACKUP_CHAT_ID_STR) if BACKUP_CHAT_ID_STR else None
except Exception:
    TG_BACKUP_CHAT_ID = None
...
BASE_RATINGS = {
    "count": 1253,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
async def show_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === مرات استخدام GO (من شيت bot_stats + التعويض التاريخي) ===
    excel_go = 0
    try:
        df_bot_stats = excel_all.get("bot_stats", pd.DataFrame())
        if df_bot_stats is not None and not df_bot_stats.empty:
            if "key" in df_bot_stats.columns and "value" in df_bot_stats.columns:
                row = df_bot_stats[df_bot_stats["key"] == "total_go_uses"]
                if not row.empty:
                    excel_go = int(row["value"].iloc[0] or 0)
    except Exception as e:
        logging.error(f"[STATS] خطأ في قراءة bot_stats من Excel: {e}")
        excel_go = 0

    total_go = BASE_STATS["go_uses"] + excel_go
    ...
    text = (
        ...
        f"{rating_info}\n\n"
        "⏳ <i>تُحدَّث هذه الأرقام تلقائيًا مع نشاط أعضاء المجتمع.</i>\n"
...
# 📌 عدّاد استخدام GO — تخزين في الذاكرة + حفظ مجمّع في شيت bot_stats
GLOBAL_GO_COUNTER = 0      # عدّاد الجلسة الحالية (للمتابعة الصحية)
GO_BUFFER = 0              # زيادات معلّقة لم تُكتب بعد في Excel
PERSISTED_GO_USES = 0      # القيمة المخزّنة فعليًا في شيت bot_stats

def _update_go_stats_sync():
    """
    زيادة عدّاد استخدام GO:
    - GLOBAL_GO_COUNTER: عدّاد الجلسة الحالية
    - GO_BUFFER: الزيادات التي سيتم فلشها لاحقًا إلى Excel بالجوب الدوري
    """
    global GLOBAL_GO_COUNTER, GO_BUFFER
    GLOBAL_GO_COUNTER += 1
    GO_BUFFER += 1
    logging.info(f"[GO STATS] buffered go usage (this_run={GLOBAL_GO_COUNTER}, pending={GO_BUFFER})")


async def update_go_stats_async():
    """غلاف async بسيط لزيادة العدّاد في الذاكرة فقط (بدون لمس Excel مباشرة)"""
    try:
...
def _flush_go_buffer_to_excel_sync():
    """
    فلش (حفظ) الزيادات المتراكمة في GO_BUFFER داخل شيت bot_stats.total_go_uses
    حتى لا نكتب على Excel مع كل استخدام /go ونسبب بطء أو تلف للملف.
    """
    global GO_BUFFER, PERSISTED_GO_USES
    delta = int(GO_BUFFER or 0)
    if delta <= 0:
        return

    # نصفر البافر أولاً، ولو فشلت الكتابة نرجّعه حتى لا نخسر الأرقام
    GO_BUFFER = 0
    try:
        try:
            excel_all = pd.read_excel("bot_data.xlsx", sheet_name=None)
        ...
        df_bot = excel_all.get("bot_stats", pd.DataFrame())

        if df_bot.empty or "key" not in df_bot.columns or "value" not in df_bot.columns:
            df_bot = pd.DataFrame([{"key": "total_go_uses", "value": 0}])

        mask = df_bot["key"] == "total_go_uses"
        if mask.any():
            current = int(df_bot.loc[mask, "value"].iloc[0] or 0)
            new_value = current + delta
            df_bot.loc[mask, "value"] = int(new_value)
        else:
            new_value = delta
            df_bot.loc[len(df_bot)] = {"key": "total_go_uses", "value": int(new_value)}
        ...
        with pd.ExcelWriter(
            ...
            df_bot.to_excel(writer, sheet_name="bot_stats", index=False)

        PERSISTED_GO_USES = int(new_value)
        logging.info(f"[GO STATS] ✅ flushed +{delta} usages to Excel (total_go_uses={new_value})")
    except Exception as e:
        # لو حصل خطأ نرجّع الزيادات للبافر حتى لا تضيع
        GO_BUFFER += delta
        logging.error(f"[GO STATS] ❌ فشل حفظ bot_stats في Excel: {e}")


async def flush_go_stats_job(context: ContextTypes.DEFAULT_TYPE):
    """
    جوب دوري لفلش عدّاد GO إلى ملف Excel.
    يُفضّل تشغيله من JobQueue كل دقيقة أو عدة دقائق.
    """
    try:
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, _flush_go_buffer_to_excel_sync)
    except Exception as e:
        logging.error(f"[GO STATS] ❌ خطأ في الجوب الدوري لحفظ bot_stats: {e}")
...
async def perform_full_backup(
    ...
    *,
    ...
    """
    إنشاء نسخة احتياطية من bot_data.xlsx وإرسالها إلى مجموعة النسخ الاحتياطي TG_BACKUP_CHAT_ID.
    - يتم فلش عداد GO إلى الإكسل قبل الإرسال حتى لا تضيع الإحصائيات.
    """
    # أولاً: فلش عداد GO (لو فيه زيادات معلّقة)
    try:
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, _flush_go_buffer_to_excel_sync)
    except Exception as e:
        logging.error(f"[BACKUP] خطأ أثناء فلش إحصائيات GO قبل النسخ الاحتياطي: {e}")

    if not TG_BACKUP_CHAT_ID:
        logging.warning("[BACKUP] لم يتم ضبط TG_BACKUP_CHAT_ID في متغيرات البيئة.")
        if triggered_by:
            try:
                await context.bot.send_message(
                    chat_id=triggered_by,
                    text="⚠️ لم يتم ضبط معرف مجموعة النسخ الاحتياطي TG_BACKUP_CHAT_ID على المنصة.",
                )
            except Exception:
                pass
        return
    ...
    if not os.path.exists(file_path):
        logging.warning(f"[BACKUP] ملف {file_path} غير موجود، لا يمكن إرسال النسخة الاحتياطية.")
        if triggered_by:
            try:
                await context.bot.send_message(
                    chat_id=triggered_by,
                    text="⚠️ ملف bot_data.xlsx غير موجود على الخادم، لا يمكن إنشاء نسخة احتياطية.",
                )
            except Exception:
                pass
        return
    ...
    caption = (
        "📦 نسخة احتياطية من بيانات نظام GO\n"
        f"السبب: {reason}\n"
        f"الوقت: {datetime.now(timezone.utc) + timedelta(hours=3)}"
    )
    ...
    try:
        ...
        logging.info("[BACKUP] ✅ تم إرسال النسخة الاحتياطية إلى مجموعة النسخ الاحتياطي.")
        if triggered_by:
            try:
                await context.bot.send_message(
                    chat_id=triggered_by,
                    text="✅ تم إنشاء النسخة الاحتياطية وإرسالها إلى مجموعة النسخ الاحتياطي.",
                )
            except Exception:
                pass
    except Exception as e:
        logging.error(f"[BACKUP] ❌ فشل إرسال النسخة الاحتياطية: {e}")
        if triggered_by:
            try:
                await context.bot.send_message(
                    chat_id=triggered_by,
                    text="❌ حدث خطأ أثناء إرسال النسخة الاحتياطية، راجع سجل الأخطاء على Render.",
                )
            except Exception:
                pass
...
async def daily_backup_job(context: ContextTypes.DEFAULT_TYPE):
    """جوب يومي يأخذ نسخة احتياطية تلقائية الساعة 4 صباحًا بتوقيت السعودية."""
    await perform_full_backup(context, triggered_by=None, reason="backup_daily_4am")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.bot_data.get("maintenance_mode"):
        ...
        with open("go-ss.jpg", "rb") as photo:
...
async def send_cost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🧩 ربط البراند بوكيله:
    # - "شيري / إكسيد" وكل الصيغ المشابهة → سنابل الحديثة
    # - BYD / جايكو / أومودا / جيتور / سواسيت → نفس الاسم (ويُجلب من BRAND_CONTACTS)
    # - لو البراند فاضي → نفترضه تابع لسنابل الحديثة (للبيانات القديمة)
    if brand_raw:
    ...
    # حذف زرّي "عرض التكلفة" و "عرض ملف الصيانة" من الرسالة الأصلية (حتى لا يتكرروا فوق)
    try:
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔹 رسالة ختامية فيها أزرار رجوع
        ...
        user_block = f"`🧑‍💼 استفسار دعم فني خاص بـ {user_name}`"
        prompt_block = (
            "💬 أهلاً بك في *مركز الدعم الفني* لبرنامج GO.\n\n"
            "✉️ يرجى كتابة استفسارك أو ملاحظتك، مع إمكانية إرفاق *ملحق واحد فقط* "
            "(صورة أو ملف أو مقطع صوتي) مع النص في كل استفسار.\n\n"
            "⚠️ *لخدمتك بشكل أدق:* نرجو إضافة **فئة السيارة والموديل والسنة** داخل الاستفسار "
            "لضمان دقة التشخيص وصحة الرد.\n\n"
            "ℹ️ إذا احتجت إرسال أكثر من ملف يمكنك إرسالها في استفسارات منفصلة "
            "حتى لا تضيع المرفقات أو يتم تجاهل بعضها.\n\n"
            f"`⏳ سيتم حفظ هذه الجلسة مؤقتاً لمتابعة رد فريق GO ({delete_time} / 🇸🇦)`"
        )
...
async def handle_send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if media:
            ...
            try:
                with open("GO-CHERY.jpg", "rb") as image:
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "backup_now":
        await query.answer("📦 جاري إنشاء النسخة الاحتياطية...", show_alert=False)
        await perform_full_backup(
            context,
            triggered_by=user_id,
            reason="backup_manual_from_control_panel",
        )
        return
...
application.add_handler(CallbackQueryHandler(
    ...
    pattern="^(ctrl_maintenance_on|ctrl_maintenance_off|reload_settings|add_admin|list_admins|clear_sessions|show_stats|self_destruct|exit_control|control_back|admins_menu|restart_session|delete_admin|broadcast_update|backup_now)$"
))
...
async def on_startup():
    ...
    # ✅ تفعيل JobQueue (تنظيف الجلسات + فلش الإحصائيات + نسخ احتياطي)
    if application.job_queue:
        ...
        # فلش عدّاد GO إلى Excel كل دقيقة (تجميع آمن للإحصائيات)
        application.job_queue.run_repeating(
            flush_go_stats_job,
        ...
        # جوب صحي اختياري كل 10 دقائق
        application.job_queue.run_repeating(
            health_log_job,
            interval=10 * 60,  # كل 10 دقائق
            first=180
        )

        # نسخ احتياطي يومي الساعة 4 فجراً بتوقيت السعودية
        try:
            saudi_tz = timezone(timedelta(hours=3))
            application.job_queue.run_daily(
                daily_backup_job,
                time=dtime(hour=4, minute=0, tzinfo=saudi_tz),
            )
        except Exception as e:
            logging.error(f"[JOBQUEUE] فشل جدولة النسخ الاحتياطي اليومي: {e}")
...
def get_update_image_path() -> Path:
    # يعثر على GO-now.jpg بجانب main.py مهما كان الـ CWD
    return Path(__file__).with_name("GO-now.jpg")
