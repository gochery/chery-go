...
async def handle_suggestion_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ بعد أول رد: ممنوع على غير نفس المشرف (بالـ ID أولاً ثم الاسم احتياط)
    ...
    if record.get("replied_by") and record.get("caption") and record.get("replied_by") != admin_name and not existing_admin_id:
        await query.answer(
            f"🟥 تم الرد على هذا الاستفسار مسبقًا من قبل: {record['replied_by']}",
            show_alert=True
        )
