...
async def send_cost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🏷 قراءة البراند من شيت الصيانة (مع افتراض سنابل الحديثة كافتراضي للبيانات القديمة)
    brand_raw = str(result.get("brand", "")).strip()
    normalized_brand = brand_raw or "سنابل الحديثة"

    # جلب بيانات الشركة والرقم من القاموس
    contact_info = BRAND_CONTACTS.get(normalized_brand, {})
