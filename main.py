...
# ✅ تهيئة المتغيرات
df_admins = df_replies = df_branches = df_maintenance = df_parts = df_manual = (
    df_independent
) = pd.DataFrame()
ALL_USERS = set()
user_sessions = {}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        delete_block = (
            f"`⏳ سيتم حذف هذا التنبيه تلقائيًا خلال 5 دقائق ({delete_time} / 🇸🇦)`"
        )
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "🔧 استعلام  قطع الغيار", callback_data=f"parts_{user_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "🚗 استعلام  الصيانة الدورية", callback_data=f"maintenance_{user_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "📘 عرض دليل المالك CHERY", callback_data=f"manual_{user_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "🛠️ المتاجر ومراكر الخدمة", callback_data=f"service_{user_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "✉️ تقديم اقتراح أو ملاحظة", callback_data=f"suggestion_{user_id}"
            )
        ],
    ]
    ...
    msg2 = await update.message.reply_text(
        "اختار الخدمة المطلوبة 🛠️ :", reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def start_suggestion_session(user_id, context):
    ...
    if (
        group_name in ["غير معروف", None] or group_id in ["غير معروف", None, user_id]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in suggestion_records:
        suggestion_id = context.user_data.get(user_id, {}).get("active_suggestion_id")
        if (
            suggestion_id
            and not suggestion_records[user_id][suggestion_id]["submitted"]
        ):
            return  # تجاهل أي رسالة أثناء جلسة اقتراح غير مكتملة
    global df_admins
    ...
    actual_user_id = context.user_data.get(admin_id, {}).get(
        "custom_reply_for", admin_id
    )
    mode = context.user_data.get(actual_user_id, {}).get(
        "action"
    ) or context.user_data.get(admin_id, {}).get("compose_mode")
    ...
    if mode in ["suggestion", "custom_reply"]:
        ...
        if mode == "suggestion":
            ...
            buttons = [
                [
                    InlineKeyboardButton(
                        "📤 إرسال الرد", callback_data="submit_admin_reply"
                    )
                ],
        ...
        if has_text and has_media:
            ...
            await message.reply_text(
                "📎 تم حفظ النص. يمكنك الآن إدخال وسائط أو الإرسال:",
                reply_markup=InlineKeyboardMarkup(buttons),
            )
        elif has_media:
            await message.reply_text(
                "🖼️ تم حفظ الوسائط. يمكنك الآن إدخال نص أو الإرسال:",
                reply_markup=InlineKeyboardMarkup(buttons),
            )
        else:
            await message.reply_text(
    ...
    if context.user_data.get(user_id, {}).get("action") == "parts" and message.text:
        ...
        for idx, row in matches.iterrows():
            station = (
                html.escape(str(row["Station Name"]))
                if pd.notna(row["Station Name"])
                else "غير معروف"
            )
            part_no = (
                html.escape(str(row["Part No"]))
                if pd.notna(row["Part No"])
                else "غير متوفر"
            )
        ...
        keyboard = [
            [InlineKeyboardButton("🗂 عرض القطع المصنفة", callback_data=callback_data)]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_manualcar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.from_user.id != user_id_from_callback:
        ...
        if context.user_data.get(user_id_from_callback, {}).get("manual_sent"):
            await query.answer(
                "❌ لا يمكنك اختيار دليل آخر في نفس الاستعلام، يرجى استخدام الأمر /go من جديد.",
                show_alert=True,
            )
            return
    ...
    if match.empty:
        ...
        msg = await query.message.reply_text(
            caption, parse_mode=constants.ParseMode.MARKDOWN
        )
        register_message(
...
async def select_car_for_parts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data[user_id][
        "search_attempts"
    ] = 0  # إعادة تعيين العداد عند كل اختيار فئة
...
async def send_part_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    part_no = (
        html.escape(str(row["Part No"])) if pd.notna(row["Part No"]) else "غير متوفر"
    )
...
async def car_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await query.edit_message_text(
        "اختر مسافة km الصيانة 🧾 :", reply_markup=InlineKeyboardMarkup(keyboard)
    )
    register_message(user_id, msg.message_id, query.message.chat_id, context)
...
async def km_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get(user_id, {}).get("km_used"):
        await query.answer(
            f"❌ عزيزي {query.from_user.full_name}، لا يمكنك فتح هذا الاستعلام مرتين بنفس الجلسة. الرجاء استخدام /go من جديد.",
            show_alert=True,
        )
        return
...
async def send_cost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get(user_id, {}).get("cost_used"):
        await query.answer(
            "❌ لا يمكنك فتح هذا الاستعلام مرتين بنفس الجلسة.", show_alert=True
        )
        return
...
async def send_brochure(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await log_event(
        update,
        f"📄 عرض ملف صيانة لـ {result['car_type']} عند {result['km_service']} كم",
    )
    # ✅ إنهاء الجلسة بعد الإرسال
    context.user_data[user_id] = {}
...
async def handle_branch_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        updated_keyboard = [
            row
            for row in keyboard
            if not any("branches_" in button.callback_data for button in row)
        ]
        await query.message.edit_reply_markup(
            reply_markup=InlineKeyboardMarkup(updated_keyboard)
        )
...
async def handle_independent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        updated_keyboard = [
            row
            for row in keyboard
            if not any("independent_" in button.callback_data for button in row)
        ]
        await query.message.edit_reply_markup(
            reply_markup=InlineKeyboardMarkup(updated_keyboard)
        )
...
async def _send_independent_results(
    update: Update, context: ContextTypes.DEFAULT_TYPE, filter_type: str = None
):
    query = update.callback_query
    await query.answer()
    ...
    for _, row in filtered.iterrows():
        ...
        try:
            if image:
                ...
                msg = await context.bot.send_message(
                    chat_id=message.chat.id, text=caption, parse_mode="HTML"
                )
        except:
            msg = await context.bot.send_message(
                chat_id=message.chat.id, text=caption, parse_mode="HTML"
    ...
    # ✅ حذف الزر المستخدم فقط (مع إبقاء الآخر)
    ...
    for _, row in results.iterrows():
        ...
        image_url = (
            row.get("image_url") if pd.notna(row.get("image_url", None)) else None
        )
    ...
    if context.user_data.get(user_id, {}).get("centers_used"):
        await query.answer(
            "❌ لا يمكنك تكرار هذا الاستعلام في نفس الجلسة، استخدم /go من جديد.",
            show_alert=True,
        )
        return
    ...
    try:
        ...
        updated_keyboard = [
            row
            for row in keyboard
            if not any("show_centers_" in btn.callback_data for btn in row)
        ]
        await query.message.edit_reply_markup(
            reply_markup=InlineKeyboardMarkup(updated_keyboard)
        )
...
# ✅ دوال عرض المراكز والمتاجر المستقلة
async def show_center_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _send_independent_results(update, context, filter_type="ورشة")
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "parts":
        keyboard = [
            [
                InlineKeyboardButton(
                    "🔍 استعلام قطع غيار استهلاكية",
                    callback_data=f"consumable_{user_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    "🌐 استعلام قطع غيار عام", callback_data=f"external_{user_id}"
                )
            ],
        ]
        msg = await query.edit_message_text(
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    car, callback_data=f"carpart_{car.replace(' ', '_')}_{user_id}"
                )
            ]
        ...
        for i, row in matches.iterrows():
            ...
            text = (
                ...
                f"<code>📌 تم العثور على نتائج بناءً على التصنيف</code>" + footer
            )
        ...
        msg = await query.edit_message_text(
            "اختر فئة السيارة 🚗 :", reply_markup=InlineKeyboardMarkup(keyboard)
        )
        register_message(user_id, msg.message_id, query.message.chat_id, context)
        ...
        suggestion_records[user_id][suggestion_id]["group_name"] = (
            chat.title if chat.title else "خاص"
        )
...
async def handle_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        user_id in suggestion_records
        and suggestion_records[user_id].get("submitted")
        and not context.user_data.get(user_id, {}).get("from_group")
    ):
        await update.message.reply_text(
            "⚠️ لا يمكنك إرسال اقتراح جديد قبل الانتهاء من الاقتراح الحالي."
        )
        return
    ...
    await update.message.reply_text(
        "✉️ يرجى كتابة اقتراحك أو إرسال صورة أو فيديو أو ملف كملاحظة."
    )
    await log_event(update, "بدأ المستخدم إرسال اقتراح أو ملاحظة")
    ...
    try:
        ...
        if media:
            ...
            with open("GO-now.jpg", "rb") as image:
                await context.bot.send_photo(
                    user_id, image, caption=user_caption, parse_mode=ParseMode.MARKDOWN
                )
...
async def auto_cancel_suggestion(context: ContextTypes.DEFAULT_TYPE):
    user_id = context.job.data.get("user_id")
    suggestion_id = context.job.data.get("suggestion_id")

    if (
        user_id in suggestion_records
        and suggestion_id in suggestion_records[user_id]
        and not suggestion_records[user_id][suggestion_id]["submitted"]
    ):
        suggestion_records[user_id].pop(suggestion_id, None)
        context.user_data[user_id].pop("active_suggestion_id", None)

        try:
            await context.bot.send_message(
                chat_id=user_id,
                text="⚠️ تم إلغاء جلسة الاقتراح تلقائيًا لعدم وجود تفاعل.",
                parse_mode="HTML",
            )
        except:
            pass
...
async def handle_control_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "📢 إشعار بتحديث البوت", callback_data="broadcast_update"
            )
        ],
    ...
    if query.data == "show_stats":
        try:
            ...
            try:
                stats_df = pd.read_excel("bot_data.xlsx", sheet_name="bot_stats")
                if "total_go_uses" in stats_df["key"].values:
                    total_go = stats_df.loc[
                        stats_df["key"] == "total_go_uses", "value"
                    ].values[0]
                else:
                    total_go = 0
            except:
...
application.add_handler(
    CallbackQueryHandler(handle_suggestion_reply, pattern=r"^reply_\d+_.+$")
)
application.add_handler(
...
application.add_handler(
    CallbackQueryHandler(
        button, pattern=r"^(parts|maintenance|consumable|external|suggestion)_\d+$"
    )
)
application.add_handler(
    CallbackQueryHandler(select_car_for_parts, pattern=r"^carpart_")
)
application.add_handler(
...
async def on_startup():
    ...
    webhook_url = (
        os.getenv("RENDER_EXTERNAL_URL") or "https://chery-go.onrender.com/webhook"
    )
