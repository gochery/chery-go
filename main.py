...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🟦 إصلاح جذري: كل استفسار جديد → تذكرة جديدة مستقلة
    suggestion_id = str(uuid4())
    user_data["active_suggestion_id"] = suggestion_id

    # تجهيز مساحة هذا المستخدم في السجلات
    ...
    # جلب الاستفسار الذي كتبه المستخدم قبل الضغط على زر (إرسال)
    old_id = user_data.get("last_preview_id")
    if old_id:
        prev = suggestion_records.get(user_id, {}).get(old_id)
        if prev:
            record["text"] = prev.get("text", "")
            record["media"] = prev.get("media", None)
    ...
    for admin_id in AUTHORIZED_USERS:
        try:
            ...
            if media:
                ...
                if mtype == "photo":
                    sent = await context.bot.send_photo(admin_id, fid,
                                                        caption=full_caption,
                                                        parse_mode=ParseMode.HTML,
                                                        reply_markup=keyboard)
                elif mtype == "video":
                    sent = await context.bot.send_video(admin_id, fid,
                                                        caption=full_caption,
                                                        parse_mode=ParseMode.HTML,
                                                        reply_markup=keyboard)
                elif mtype == "document":
                    sent = await context.bot.send_document(admin_id, fid,
                                                           caption=full_caption,
                                                           parse_mode=ParseMode.HTML,
                                                           reply_markup=keyboard)
                elif mtype == "voice":
                    sent = await context.bot.send_voice(admin_id, fid,
                                                        caption=full_caption,
                                                        parse_mode=ParseMode.HTML,
                                                        reply_markup=keyboard)
            else:
                ...
                sent = await context.bot.send_message(admin_id,
                                                      text=full_caption,
                                                      parse_mode=ParseMode.HTML,
                                                      reply_markup=keyboard)
