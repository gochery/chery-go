...
async def start_recommendation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data[admin_id]["reco_entities"] = None  # 👈 جديد
    ...
    # 🔹 زر إلغاء التوصية + الرجوع للقائمة + حذف الرسالة
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("❌ إلغاء التوصية والرجوع للقائمة", callback_data="cancel_reco")]
    ])
...
async def handle_recommendation_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # حفظ بيانات التوصية في user_data
    ud["reco_text"] = text
    ud["reco_media"] = media
    ...
    preview_caption = (
        ...
        "اختر طريقة الإرسال:\n"
        "• «بث على جميع المجموعات» لإرسالها لكل القروبات المسجلة.\n"
        "• «اختيار مجموعات يدوياً» لتحديد القروبات المستهدفة.\n"
        "أو أرسل رسالة جديدة لتعديل النص قبل الإرسال."
    )

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📡 بث على جميع المجموعات", callback_data="reco_broadcast")],
        [InlineKeyboardButton("📂 اختيار مجموعات يدوياً", callback_data="reco_choose_groups")],
...
async def broadcast_recommendation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """بث التوصية على جميع المجموعات + إشعار جميع المشرفين مع تثبيت الرسالة"""
    query = update.callback_query
    admin_id = query.from_user.id
    admin_name = query.from_user.full_name
    ...
    entities = ud.get("reco_entities")  # تنسيقات تيليجرام (رابط مخفي، بولد، ...)
    ...
    for chat_id in targets:
        try:
            # تأكد أن البوت مشرف في المجموعة قبل الإرسال (للسماح بالتثبيت أيضاً)
            try:
            ...
            if media:
                ...
                # لو فيه تنسيقات (ومنها رابط مخفي)، نمررها كـ caption_entities
                if mtype == "photo":
            ...
            # محاولة تثبيت الرسالة في كل مجموعة
            if sent_msg is not None:
                try:
                    ...
                    logging.warning(f"[RECO PIN] تم الإرسال بنجاح لكن فشل تثبيت الرسالة في {chat_id}: {e}")
    ...
    for aid in AUTHORIZED_USERS:
        try:
            if media and media.get("type") == "photo":
                await context.bot.send_photo(aid, media["file_id"], caption=admin_notification_caption)
            else:
    ...
    ud["reco_text"] = None
    ud["reco_media"] = None
    ud["reco_entities"] = None  # لو مخزّن التنسيقات


async def broadcast_recommendation_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """بث التوصية على المجموعات المحددة يدوياً مع تثبيت الرسالة"""
    query = update.callback_query
    admin_id = query.from_user.id
    admin_name = query.from_user.full_name
    ...
    text = ud.get("reco_text")
    media = ud.get("reco_media")
    entities = ud.get("reco_entities")
    selected = ud.get("reco_selected") or []

    if (not text and not media) or not selected:
        await query.answer("يرجى كتابة التوصية أولاً ثم اختيار مجموعة واحدة على الأقل للبث.", show_alert=True)
        return

    await query.answer("📡 جاري بث التوصية على المجموعات المحددة...", show_alert=False)

    targets = set(int(cid) for cid in selected)
    sent = failed = skipped = 0
        ...
        try:
            ...
            if sent_msg is not None:
                try:
                    ...
                    logging.warning(f"[RECO PIN SELECTED] تم الإرسال بنجاح لكن فشل تثبيت الرسالة في {chat_id}: {e}")

            sent += 1
        except Exception as e:
            logging.warning(f"[RECO BROADCAST SELECTED] فشل إرسال التوصية إلى {chat_id}: {e}")
            failed += 1

    summary = (
        "📡 تمت عملية بث التوصية الفنية على المجموعات المحددة.\n\n"
        f"✅ تم الإرسال إلى: {sent} مجموعة\n"
        f"⏭️ تم التخطي في: {skipped} مجموعة (البوت ليس مشرفاً)\n"
        f"⚠️ فشل الإرسال في: {failed} مجموعة"
    )
    ...
    group_title = ud.get("group_title", "—")
    admin_notification_caption = (
        "📡 تمت عملية بث توصية فنية جديدة (على مجموعات محددة).\n\n"
        f"👤 الناشر: {admin_name}\n"
        f"👥 المجموعة التابعة له: {group_title}\n\n"
        "📄 نص التوصية:\n"
        f"{text or '— التوصية بدون نص (وسائط فقط) —'}"
    ...
    for aid in AUTHORIZED_USERS:
        try:
            ...
            logging.warning(f"[RECO NOTIFY ADMIN SELECTED] فشل إشعار المشرف {aid}: {e}")

    # لا نحذف نص التوصية بالكامل، لكن ننهي وضع الاختيار
    ud["reco_mode"] = None
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ملاحظة: لا نتدخل إذا كان للمستخدم في وضع آخر نشط (بحث قطع، دعم فني، ...).
    if admin_id in AUTHORIZED_USERS:
        ...
        # ✅ أولوية للتوصية الفنية متى ما كانت مفعّلة، حتى لو كان فيه action أو compose_mode قديم
        if reco_mode == "awaiting_reco" and chat.type == "private":
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    thank_you_message += (
        "✅ تم إرسال الاستفسار بنجاح إلى فريق الدعم .\n"
...
async def handle_send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_additional:
        ...
        user_caption = (
            ...
            f"\u200F💬 *رد المشرف:*\n"
            f"```{reply_text.strip()}```\n\n"
            f"\u200F🤖 *شكرًا لثقتك بفريق الصيانة GO.*"
        )
...
async def submit_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ معالجة الوسائط من المشرف فقط (بدون وراثة لأي وسائط قديمة)
    media = context.user_data[admin_id].get("compose_media")

    # ✅ الآن: لا نسمح بالرد الفارغ تماماً (لا نص ولا وسائط)
    ...
    # نضيف فقرة "رد المشرف" فقط لو فيه نص فعلي
    if reply_text:
        user_caption += f"\u200F💬 *رد المشرف:*\n```{reply_text}```\n\n"

    user_caption += f"\u200F🤖 *شكرًا لثقتك بفريق الصيانة GO.*"
    ...
    # برضه هنا: نضيف "رد المشرف" فقط لو فيه نص
    if reply_text:
...
async def save_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if already_rated:
            ...
            # ✅ تنبيه منبثق يحتوي نص الشكر الكامل مع الاسم
            alert_text = (
...
def collect_target_chat_ids(context: ContextTypes.DEFAULT_TYPE) -> set[int]:
    """يجمع chat_id من:
       1) شيت group_logs (كل المجموعات/القنوات المسجلة)
       2) user_sessions (جلسات التفاعل)
    """
    targets: set[int] = set()
    global df_group_logs
    ...
    try:
        ...
        logging.warning(f"[RECO TARGETS] فشل تحميل المجموعات من user_sessions: {e}")

    logging.info(f"[RECO TARGETS] عدد المجموعات المتاحة للبث: {len(targets)}")
    return targets
...
def _build_reco_groups_page(admin_id: int, context: ContextTypes.DEFAULT_TYPE, page_size: int = 5):
    """يبني نص وقائمة أزرار لاختيار المجموعات يدوياً للتوصية"""
    ud = context.user_data.setdefault(admin_id, {})
    groups = ud.get("reco_targets") or []
    selected = set(ud.get("reco_selected") or [])
    page = ud.get("reco_page") or 0

    total = len(groups)
    if total == 0:
        text = "⚠️ لا توجد حالياً أي مجموعات أو قنوات مسجلة يمكن البث إليها."
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ إغلاق", callback_data="reco_cancel")]
        ])
    ...
    page_items = groups[start_idx:end_idx]

    lines = [
        "📂 *اختيار المجموعات لبث التوصية*",
        "",
        f"الصفحة {page + 1} من {max_page + 1} • إجمالي المجموعات: {total}",
        "",
        "اضغط على اسم المجموعة لتفعيل / إلغاء تحديدها:",
        "",
    ]
    ...
    for g in page_items:
        cid = g.get("id")
        title = g.get("title") or f"مجموعة {cid}"
        mark = "✅" if cid in selected else "☑️"
        lines.append(f"{mark} {title} (`{cid}`)")
        rows.append([
            InlineKeyboardButton(f"{mark} {title[:40]}", callback_data=f"reco_tgl_{cid}")
        ])
    ...
    # أزرار الإجراءات
    rows.append([InlineKeyboardButton("📡 بث على المجموعات المحددة", callback_data="reco_send_selected")])
    rows.append([InlineKeyboardButton("📡 بث على جميع المجموعات", callback_data="reco_broadcast")])
...
async def start_reco_group_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """فتح شاشة اختيار المجموعات يدوياً للمشرف"""
    query = update.callback_query
    admin_id = query.from_user.id
    ...
    ud = context.user_data.setdefault(admin_id, {})
    # تأكد أن التوصية موجودة
    if not ud.get("reco_text") and not ud.get("reco_media"):
        await query.answer("لا توجد توصية جاهزة للبث. يرجى إرسال التوصية أولاً.", show_alert=True)
        return

    # تجهيز قائمة المجموعات
    _prepare_reco_targets_for_admin(admin_id, context)
    groups = ud.get("reco_targets") or []
    if not groups:
        await query.answer("لا توجد أي مجموعات مسجلة حالياً للبث.", show_alert=True)
        return

    ud["reco_page"] = 0
    text, keyboard = _build_reco_groups_page(admin_id, context)
...
async def toggle_reco_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """تبديل حالة تحديد مجموعة معينة للتوصية"""
    query = update.callback_query
    admin_id = query.from_user.id
    ...
    data = (query.data or "").split("_")
    if len(data) < 3:
        await query.answer("بيانات اختيار غير صالحة.", show_alert=True)
        return

    try:
        chat_id = int(data[2])
    except ValueError:
        await query.answer("معرّف مجموعة غير صالح.", show_alert=True)
        return
...
async def change_reco_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """التنقل بين صفحات المجموعات في شاشة اختيار التوصية"""
    query = update.callback_query
    admin_id = query.from_user.id
    ...
    direction = (query.data or "").split("_")[-1]
    ud = context.user_data.setdefault(admin_id, {})
    page = ud.get("reco_page") or 0
...
application.add_handler(
    CallbackQueryHandler(
        ...
        pattern="^(ctrl_maintenance_on|ctrl_maintenance_off|reload_settings|add_admin|list_admins|clear_sessions|self_destruct|control_back|admins_menu|restart_session|delete_admin|broadcast_update|ctrl_backup)$"
    )
)
...
# ✅ اختيار المجموعات يدوياً + التبديل + التنقل بين الصفحات + إرسال المحدد
application.add_handler(CallbackQueryHandler(start_reco_group_choice, pattern=r"^reco_choose_groups$"))
application.add_handler(CallbackQueryHandler(toggle_reco_group, pattern=r"^reco_tgl_-?\d+$"))
application.add_handler(CallbackQueryHandler(change_reco_page, pattern=r"^reco_page_(next|prev)$"))
application.add_handler(CallbackQueryHandler(broadcast_recommendation_selected, pattern=r"^reco_send_selected$"))
# ✅ الإرسال العام + الإلغاء
...
ication.add_handler(CallbackQueryHandler(team_reply_existing_thread, pattern=r"^team_reply_\d+$"))
