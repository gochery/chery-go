...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🎯 عند بدء جلسة GO جديدة يتم إعادة ضبط عدّاد الاستفسارات
    context.user_data[user_id]["suggestion_count"] = 0
    context.user_data[user_id]["active_suggestion_id"] = None

    # ✅ منع إرسال أي استفسار جديد مباشرة بدون الضغط على زر "إرسال استفسار آخر"
    context.user_data[user_id]["can_send_suggestion"] = True
    ...
    # ✅ بعد الانتهاء، نجعل الجلسة صالحة لإرسال الاستفسارات فقط عبر زر "إرسال استفسار آخر"
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تتبع عدد المحاولات
    attempt_count = user_data.get("attempt_count", 0)
    active_suggestion_id = user_data.get("active_suggestion_id")

    # ✅ من الخاص: التحقق من الجلسة والمحاولات
    if chat.type == "private":
        # إذا أنهى تذكرته الأولى ولم يبدأ جلسة جديدة عبر الزر، تجاهل الرسائل النصية
        if attempt_count >= 1 and active_suggestion_id is None:
            return  # تجاهل الرسالة دون أي رد

        # إذا استهلك كل المحاولات
        if attempt_count >= 3:
            await update.message.reply_text(
                f"⚠️ تم استنفاد جميع محاولاتك الثلاث. لإرسال استفسارات جديدة، يرجى إعادة بدء GO."
            )
            return

    # 🧾 التعامل مع الرسائل من المجموعات
    if chat.type != "private":
        context.bot_data[user_id] = {
        ...
        keys_to_remove = [key for key in user_data if key.startswith("image_opened_")]
        for key in keys_to_remove:
            del user_data[key]

        # إرسال بانر GO / زر الانطلاق
        await start(update, context)

        # تحديث سجل المجموعة في الخلفية
        try:
    ...
    # ✅ من الخاص مع جلسة غير صالحة أو مستخدم غير مصرح له
    if chat.type == "private" and not user_data.get("session_valid") and user_id not in AUTHORIZED_USERS:
        now_saudi = datetime.now(timezone.utc) + timedelta(hours=3)
        delete_time = (now_saudi + timedelta(minutes=15)).strftime("%I:%M %p")
    ...
    # ✅ من الخاص مع جلسة صالحة أو مشرف → استدعاء start للترحيب والقائمة
    await start(update, context)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔹 رسائل القطع داخل التصنيف
        ...
        # 🚫 منع المستخدم من استخدام زر "إرسال استفسار آخر" بعد 3 مرات
        if context.user_data[user_id].get("suggestion_count", 0) >= 3:
            real_name = query.from_user.full_name
            await query.answer(
                f"⚠️ عزيزي {real_name}\nلقد استخدمت خدمة إرسال الاستفسار عدة مرات.\n"
                "يرجى العودة إلى المجموعة وكتابة GO لبدء جلسة دعم جديدة.",
                show_alert=True
            )
            return
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.user_data.get(user_id, {}).get("can_send_suggestion", False):
        await query.answer("⚠️ لا يمكنك إرسال استفسار جديد إلا بعد الضغط على زر 'إرسال استفسار آخر'.", show_alert=True)
        return
    ...
    # 🎯 زيادة عدّاد محاولات المستخدم
    context.user_data[user_id]["suggestion_count"] = context.user_data[user_id].get("suggestion_count", 0) + 1

    # 🎯 بعد الإرسال، يجب الضغط على زر "إرسال استفسار آخر" قبل السماح بالإرسال مرة أخرى
    context.user_data[user_id]["can_send_suggestion"] = False
