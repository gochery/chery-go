        ...
        try:
            if isinstance(image_url, str) and image_url.strip().lower().startswith("http"):
                msg = await context.bot.send_photo(
                    ...
                    caption=text,
                    parse_mode=constants.ParseMode.HTML,  # 🆕 تغيير إلى HTML
                )
            else:
                msg = await query.message.reply_text(
                    text,
