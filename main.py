...
# ✅ حذف السجل إذا تجاوز 1 ميغابايت
if os.path.exists(log_path) and os.path.getsize(log_path) > 1 * 1024 * 1024:
    with open(log_path, "w", encoding="utf-8") as f:
        f.write("🧹 [LOG RESET] تم مسح السجلات تلقائيًا لتوفير المساحة\n")


# ✅ تنسيق مخصص لتوقيت السعودية
class SaudiFormatter(logging.Formatter):

    def formatTime(self, record, datefmt=None):
        sa_time = datetime.fromtimestamp(record.created,
                                         timezone.utc) + timedelta(hours=3)
        if datefmt:
            return sa_time.strftime(datefmt)
        return sa_time.isoformat()


# ✅ تحسين شكل السجلات وتسجيلها في ملف خارجي log.txt يدعم العربية
logging.basicConfig(
    level=logging.INFO,
    format=
    "\n🟢 [LOG] 📅 %(asctime)s | 📂 %(filename)s:%(lineno)d\n🔹 %(levelname)s - %(message)s\n",
    datefmt="%Y-%m-%d %H:%M:%S",
    encoding="utf-8",
    filename="log.txt",
    filemode="a",
...
# ✅ تطبيق تنسيق توقيت السعودية على كل handler
...
application.job_queue.run_repeating(cleanup_old_sessions,
                                    interval=3600,
                                    first=0)
...
def register_message(user_id,
    ...
    if not skip_delete and context and hasattr(
            context, "job_queue") and context.job_queue:
        try:
            context.job_queue.run_once(schedule_delete_message,
                                       timedelta(minutes=5),
                                       data={
                                           "user_id": user_id,
                                           "message_id": message_id,
...
async def schedule_delete_message(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.delete_message(chat_id=chat_id,
                                         message_id=message_id)
        logging.info(
            f"[DELETE] 🗑️ تم حذف الرسالة رقم {message_id} للمستخدم {user_id}")
    except Exception as e:
        logging.warning(
...
async def log_event(update: Update, message: str, level="info"):
    ...
    log_msg = (f"{timestamp:%Y-%m-%d %H:%M:%S} | "
               f"📩 من: [{user.full_name}] | "
               f"🆔 المستخدم: {user.id} | "
               f"📣 المحادثة: {chat.id} | "
               f"📝 {message}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.bot_data.get("maintenance_mode"):
        ...
        context.job_queue.run_once(lambda c: c.bot.delete_message(
    ...
    group_title = chat.title if chat.title else "خاص"
    group_id = chat.id if chat.type != "private" else "غير معروف"

    # ✅ سجل دخول المستخدم للتيرمنال
    print(f"🟢 start | من: {chat.type} | 🧑 المستخدم: {user_name} | 🆔 {user_id} | 🏘️ المجموعة: {group_title} | 🔢 {group_id}")
    ...
    # ✅ التعرف على كلمات التفعيل الإضافية مثل go
    text = update.message.text.strip().lower() if update.message and update.message.text else ""

    # ✅ تجاوز الشرط إن كان المستخدم فعّل الجلسة من go
    if context.user_data.get(user_id, {}).get("session_valid"):
        started_properly = True
    else:
        started_properly = (text in ["go", "/go", "/start", "start", "/start go"] or "manualid_" in text)

    allowed_manual = update.message and "manualid_" in text and context.user_data.get(
        user_id, {}).get("allow_manualid")
    ...
    if chat.type == "private" and not allowed_manual and (
        ...
        # 🔴 الرسالة الصارمة - مستخدم تطفل على زر انطلق لا يخصه
            ...
            alert_message = (
                ...
                "🔐 هذه الخدمة مخصصة فقط لمن بدأ الجلسة من المجموعة بنفسه ولضمان الخصوصية.\n"
    ...
    if update.message and "manualid_" in update.message.text:
        try:
            ...
            msg = await update.message.reply_text(
                "🚫 دليل هذه السيارة غير متاح أو الملف غير صالح.")
            register_message(user_id, msg.message_id, chat_id, context)
            return
    ...
    if chat_id < 0:
        ...
        keyboard = [[
            InlineKeyboardButton("🚀  انطلق  مع  برنامج  GO", url=link)
        ]]
    ...
    keyboard = [[
        InlineKeyboardButton("🔧 استعلام  قطع الغيار",
                             callback_data=f"parts_{user_id}")
    ],
                [
                    InlineKeyboardButton(
                        "🚗 استعلام  الصيانة الدورية",
                        callback_data=f"maintenance_{user_id}")
                ],
                [
                    InlineKeyboardButton("📘 عرض دليل المالك CHERY",
                                         callback_data=f"manual_{user_id}")
                ],
                [
                    InlineKeyboardButton("🛠️ المتاجر ومراكر الخدمة",
                                         callback_data=f"service_{user_id}")
                ],
                [
                    InlineKeyboardButton("✉️ تقديم اقتراح أو ملاحظة",
                                         callback_data=f"suggestion_{user_id}")
                ]]
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    group_title = chat.title if chat.title else "غير معروف"
    group_id = chat.id if chat.type != "private" else "خاص"

    # ✅ سجل في التيرمنال بكل التفاصيل
    print(f"🟢 handle_go_text | من: {chat.type} | 🧑 المستخدم: {user_name} | 🆔 {user_id} | 🏘️ المجموعة: {group_title} | 🔢 {group_id}")
    ...
    if chat.type != "private":
        ...
        logging.info(
            f"[GO من المجموعة] سجلنا بيانات المجموعة {group_title} / {chat_id} للمستخدم {user_name}"
        )
    ...
    if chat.type == "private" and (not context.user_data.get(
            user_id,
        {}).get("session_valid")) and user_id not in AUTHORIZED_USERS:
        now_saudi = datetime.now(timezone.utc) + timedelta(hours=3)
        delete_time = (now_saudi + timedelta(minutes=5)).strftime("%I:%M %p")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    import sqlite3  # لضمان وجود الاستيراد
    ...
    if action == "awaiting_admin_removal":
        try:
            ...
            # حذف من قاعدة البيانات SQLite
            conn = sqlite3.connect("bot_data.db")
            cursor = conn.cursor()
            cursor.execute("DELETE FROM managers WHERE manager_id = ?", (target_id,))
            conn.commit()
            conn.close()
    ...
    if action == "awaiting_new_admin_id":
        try:
            ...
            # إضافة إلى القائمة الحية
            AUTHORIZED_USERS.append(new_admin_id)

            # إضافة إلى قاعدة البيانات SQLite
            conn = sqlite3.connect("bot_data.db")
            cursor = conn.cursor()
            cursor.execute("INSERT OR IGNORE INTO managers (manager_id) VALUES (?)", (new_admin_id,))
            conn.commit()
            conn.close()
            ...
            await message.reply_text(f"❌ فشل أثناء إضافة المشرف:\n{e}")
...
async def handle_manualcar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    caption = (
        ...
        f"🔄 سيتم نقلك إلى المحادثة الخاصة لعرض الدليل.\n"
        f"`⏳ سيتم حذف هذا الاستعلام تلقائيًا خلال 5 دقائق ({delete_time} / 🇸🇦)`"
    )
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "list_admins":
        try:
            conn = sqlite3.connect("bot_data.db")
            cursor = conn.cursor()
            rows = cursor.execute("SELECT manager_id FROM managers").fetchall()
            conn.close()

            display_rows = []
            for i, (id_,) in enumerate(rows):
                try:
                    user = await context.bot.get_chat(id_)
                    name = user.full_name
                except:
                    name = "❓ غير معروف"
                display_rows.append(f"{i+1}. {name}\n🆔 `{id_}`")
    ...
    if query.data == "reload_settings":
        try:
            ...
            await query.message.edit_text("✅ تم إعادة تحميل قائمة المشرفين من قاعدة البيانات.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ عودة", callback_data="control_back")]]))
        except Exception as e:
...
async def handle_add_admin_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    import sqlite3  # تأكد من وجود هذا الاستيراد أعلى الملف
    user_id = update.effective_user.id
    message = update.message
    ...
    try:
        ...
        await message.reply_text(f"❌ حدث خطأ أثناء الحفظ:\n{e}")
...
application.add_handler(
    CallbackQueryHandler(handle_cancel, pattern=r"^cancel_"))
...
async def webhook_handler(request: Request):
    try:
        ...
        print("✅ تم استقبال التحديث من Telegram:", json_data)
        update = Update.de_json(json_data, application.bot)
        await application.process_update(update)
        return {"ok": True}
    except Exception as e:
        print("❌ خطأ أثناء تنفيذ webhook:", str(e))
