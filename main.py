...
BASE_RATINGS = {
    "count": 1247,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
DEALER_FOR_BRAND = {
    ...
    # جيتور – أي صيغة تكتبها في شيت الصيانة
    "جيتور": "جيتور",
    "JETOUR": "جيتور",
    "جيتور JETOUR": "جيتور",   # لو تستخدمها بهذي الصيغة في الإكسل
}
...
async def show_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    صفحة إحصائيات GO + فتح التقييم في نفس الشاشة (نسخة HTML منسقة)
    """
    ...
    user_name_raw = user.full_name أو "الصديق"
    ...
    # === التقييمات ===
    rating_info = (
        '⭐ <span style="color:#8e8e8e;">لا توجد تقييمات مسجلة حاليًا</span>'
    )
    try:
        ...
        if total_ratings_display > 0:
            # حساب متوسط مجمّع بين القديم والجديد
            ...
            rating_info = (
                "⭐ التقييمات:\n"
                f'عدد المقيمين: <span style="color:#8e8e8e;">{total_ratings_display}</span>\n'
                f'متوسط التقييم: <span style="color:#8e8e8e;">{combined_avg} {stars}</span>'
            )
    except Exception:
        pass
    ...
    # === بناء نص الإحصائيات (HTML منسق) ===
    text = (
        "<b>🧑‍💼 تقرير إحصائيات نظام GO</b>\n"
        f'للمستخدم: <span style="color:#8e8e8e;">{user_name_safe}</span>\n\n'
        "<b>📊 ملخص عام:</b>\n"
        f'👥 إجمالي المستخدمين: <span style="color:#8e8e8e;">{total_users}</span>\n'
        f'🏡 عدد القروبات المستخدمة للنظام: <span style="color:#8e8e8e;">{total_groups}</span>\n'
        f'🚀 إجمالي استخدام /go: <span style="color:#8e8e8e;">{total_go}</span>\n\n'
        f"{rating_info}\n\n"
        "⏳ يتم تحديث هذه الأرقام تلقائيًا مع نشاط المستخدمين "
        f'(<span style="color:#8e8e8e;">{refresh_time} / 🇸🇦</span>)\n\n'
        "<span>🔹 فريق GO يشكرك على الثقة والدعم المستمر.</span>"
    )

    # لوحة أزرار التقييم أسفل نفس الرسالة
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        header = (
            "🧑‍💼 استعلام خاص بـ: "
            f'<span style="color:#8e8e8e;">{user_name_safe}</span>\n'
            "🚗 فئة السيارة: "
            f'<span style="color:#8e8e8e;">{selected_car_safe}</span>\n\n'
        )
        results_header = (
        ...
        note_line = (
            ...
            '💡 يمكن عرض صور قطع الغيار بالتفصيل عبر خيار قائمة التصنيف داخل خدمة قطع الغيار.'
            '</span>'
        )
