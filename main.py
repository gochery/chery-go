...
def build_main_menu_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    # ✅ زر "دعوة فريق GO للنقاش" يظهر للمشرفين فقط (الموجودين في AUTHORIZED_USERS)
    if user_id in AUTHORIZED_USERS:
        keyboard.insert(
            -1,  # قبل زر التقييم مباشرة
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ تحديث عداد استخدام go في الخلفية (بدون تعطيل رسالة الترحيب)
    try:
