...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  # ✅ إذا النداء من مجموعة: نرسل بانر ترحيب صورة فقط (بدون فيديو)
if chat_id < 0:
    ...
    photo_path = "GO-CHERY.JPG"   # صورة الترحيب الوحيدة
    ...
    try:
        # 👉 إرسال صورة فقط — بدون فيديو نهائيًا
    ...
    if (
        ...
        for idx, (_, row) in enumerate(matches.iterrows(), start=1):
            ...
            if price:
                ...
                # نضمن أن كلمة ريال في النهاية فقط
            ...
            # نجمع أسطر النتيجة الواحدة مع بعض
            lines.append("\n".join(line_parts))

        # نفصل بين كل نتيجة وأخرى بسطرين خفاف
        body = "\n\n".join(lines)
