...
async def send_cost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🏷 نقرأ البراند من جلسة المستخدم (maintenance_brand_choice)
    user_data = context.user_data.setdefault(user_id, {})
    session_brand_raw = str(user_data.get("brand", "")).strip()

    dealer_key = None

    if session_brand_raw:
        upper = session_brand_raw.upper()
        ...
        # جايكو
        elif "جايكو" in session_brand_raw:
            dealer_key = "جايكو"

        # أومودا
        elif "أومودا" in session_brand_raw or "OMODA" in upper:
            dealer_key = "أومودا"

        # جيتور
        elif "جيتور" in session_brand_raw or "JETOUR" in upper:
            dealer_key = "جيتور"

        # سوايست / سواسيت (اختلاف كتابة)
        elif "سوايست" in session_brand_raw or "سواسيت" in session_brand_raw:
            dealer_key = "سواسيت"

        # شيري / اكسيد → سنابل الحديثة
        elif "شيري" in session_brand_raw or "اكسيد" in session_brand_raw or "CHERY" in upper or "EXEED" in upper:
            dealer_key = "سنابل الحديثة"

        else:
            # أي براند آخر مستقبلاً نستخدم اسمه كما هو (وتضيفه أنت في القاموس لاحقاً)
            dealer_key = session_brand_raw

    # لو ما في براند في الجلسة (حالات قديمة) نخليها سنابل الحديثة افتراضياً
    if not dealer_key:
        dealer_key = "سنابل الحديثة"
