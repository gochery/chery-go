...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ user_data الصحيح (قاموس المستخدم نفسه)
    ...
    if chat.type == "private" and not user_state.get("session_valid") and user_id not in AUTHORIZED_USERS:
    ...
    keys_to_remove = [key for key in list(user_state.keys()) if str(key).startswith("image_opened_")]
    ...
    # ✅ استرجاع/تثبيت بيانات المجموعة
    group_title = user_state.get("group_title", "غير معروف")
    group_id = user_state.get("group_id", user_id)
    previous_user_name = user_state.get("user_name", user_name)

    # ✅ إذا كان المستخدم انتقل للخاص: استرجاع من bot_data (احتياط)
    if chat_id > 0 and user_id in context.bot_data:
        bot_data = context.bot_data[user_id]
        user_state.update(bot_data)
        del context.bot_data[user_id]

        group_title = bot_data.get("group_title", group_title)
        group_id = bot_data.get("group_id", group_id)
        previous_user_name = bot_data.get("user_name", previous_user_name)
    ...
    if chat_id < 0:
        # ✅ ثبّت بيانات المجموعة للمستخدم (حتى لو start نودي مباشرة من المجموعة)
        group_title_now = update.effective_chat.title or "غير معروف"
        group_id_now = chat_id

        context.bot_data[user_id] = {
            "group_title": group_title_now,
            "group_id": group_id_now,
            "user_name": user_name
        }
        ...
        user_state.update({
            ...
            "group_id": group_id_now,
            "user_name": user_name,
            "final_group_name": group_title_now,
            "final_group_id": group_id_now,
            "session_valid": True
        })
    ...
    user_state.pop("suggestion_used", None)
    user_state.pop("search_attempts", None)
    ...
    for key in list(user_state.keys()):
        if str(key).startswith("image_opened_") or str(key).endswith("_used") or str(key).endswith("_sent"):
    ...
    for key in list(user_state.keys()):
        if str(key).startswith("cat_used_"):
            user_state.pop(key, None)

    user_state["session_valid"] = False
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type != "private":
        group_title = getattr(chat, "title", None) or "غير معروف"
        group_id = chat.id

        # ✅ خزّن بيانات المجموعة في bot_data (احتياط إذا فقدت user_data بعد انتقاله للخاص)
        context.bot_data[user_id] = {
            "group_title": group_title,
            "group_id": group_id,
            "user_name": user_name,
        ...
        # ✅ ثبّت بيانات المجموعة داخل user_data للمستخدم (المصدر الأساسي)
        user_state["group_title"] = group_title
        user_state["group_id"] = group_id
        user_state["final_group_name"] = group_title
        user_state["final_group_id"] = group_id
        user_state["user_name"] = user_name

        # ✅ إنشاء جلسة مؤقتة صالحة لمرة واحدة فقط
        user_state["session_valid"] = True

        # تنظيف مفاتيح الصور القديمة (إن وجدت)
        keys_to_remove = [k for k in list(user_state.keys()) if str(k).startswith("image_opened_")]
        ...
        try:
            asyncio.create_task(update_group_logs(group_id, group_title or "", context))
        except Exception as e:
            logging.warning(f"[GROUP_LOGS] فشل جدولة تحديث group_logs للقروب {group_id}: {e}")

        logging.info(f"[GO من المجموعة] سجلنا بيانات المجموعة {group_title} / {group_id} للمستخدم {user_name}")
        return
