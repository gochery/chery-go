...
BRAND_CONTACTS = {
    # هذا مفتاح الوكيل نفسه (سنابل الحديثة)
    "سنابل الحديثة": {
        # انتبه: بدون كلمة "شركة" هنا، لأنها تنضاف في النص لاحقاً
        "company": "سنابل الحديثة",
        "phone": "8002440228",
    },

    # وكيل جيتور – بدون كلمة "شركة" في القيمة
    "جيتور": {
    ...
    # باقي البراندات (جاهزة لو حبيت تعبيها لاحقاً)
    "BYD": {
        "company": "",
        "phone": "",
    },
...
DEALER_FOR_BRAND = {
    # شيري / إكسيد (كلها نفس الوكيل)
    "CHERY": "سنابل الحديثة",
    "EXEED": "سنابل الحديثة",

    # لو استخدمت عربي في أي مكان مستقبلاً (ما يضر نخليها موجودة)
    "شيري / إكسيد": "سنابل الحديثة",
    "شيري": "سنابل الحديثة",
    "إكسيد": "سنابل الحديثة",

    # جيتور – أياً كانت الصيغة في الإكسل
    "JETOUR": "جيتور",
    "جيتور": "جيتور",
    "جيتور JETOUR": "جيتور",
}
...
async def update_group_logs_async(chat):
    ...
    try:
        ...
        # كتابة متسلسلة بدون أي ثريد
        with pd.ExcelWriter("bot_data.xlsx",
                            engine="openpyxl",
                            mode="a",
...
def _update_all_users_log_sync():
    ...
    try:
        ...
        logging.error(f"[SAVE USERS] فشل حفظ all_users_log: {e}")


async def update_all_users_log_async():
    try:
        _update_all_users_log_sync()
    except Exception as e:
        logging.error(f"[SAVE USERS] فشل حفظ all_users_log في الخلفية: {e}")

async def update_all_users_log_async():
    """
    غلاف async، لكنه الآن يستدعي الدالة مباشرة بدون run_in_executor
    """
    try:
        _update_all_users_log_sync()
    except Exception as e:
...
def _update_go_stats_sync():
    ...
    try:
        ...
        logging.error(f"[SAVE STATS] فشل حفظ عدد /go: {e}")

def _update_go_stats_sync():
    """تحديث عداد استخدام go داخل bot_stats"""
    try:
        stats_df = pd.read_excel("bot_data.xlsx", sheet_name="bot_stats")
    except Exception:
        stats_df = pd.DataFrame(columns=["key", "value"])

    # إذا موجود يزداد واحد
    mask = stats_df["key"] == "total_go_uses"
    if mask.any():
        stats_df.loc[mask, "value"] = stats_df.loc[mask, "value"].astype(int) + 1
    else:
        stats_df = pd.concat(
            [stats_df, pd.DataFrame([{"key": "total_go_uses", "value": 1}])],
            ignore_index=True
...
async def update_go_stats_async():
    """غلاف async لكن يشغل sync مباشرة بدون thread"""
    try:
        _update_go_stats_sync()
    except Exception as e:
        logging.error(f"[GO STATS] فشل حفظ عدد استخدام go: {e}")
...
def _write_health_log_sync():
    ...
    try:
        ...
        mask = stats_df["key"] == "total_go_uses"
        if mask.any():
            total_go = int(stats_df.loc[mask, "value"].iloc[0])
    except Exception:
        pass
...
async def health_log_job(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logging.error(f"[HEALTH LOG] خطأ أثناء كتابة health_log: {e}")
