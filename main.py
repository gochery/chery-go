...
# ✅ دالة البدء async
aysnc def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def handle_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [
            InlineKeyboardButton("⭐", callback_data=f"ratingval_1_{user_id}"),
            InlineKeyboardButton("⭐⭐", callback_data=f"ratingval_2_{user_id}"),
            InlineKeyboardButton("⭐⭐⭐", callback_data=f"ratingval_3_{user_id}"),
            InlineKeyboardButton("⭐⭐⭐⭐", callback_data=f"ratingval_4_{user_id}"),
            InlineKeyboardButton("⭐⭐⭐⭐⭐", callback_data=f"ratingval_5_{user_id}")
        ]
    ]
...
async def save_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if os.path.exists(ratings_file):
            ...
            df_ratings = pd.DataFrame(columns=["user_id", "name", "rating", "timestamp"])

        df_ratings = pd.concat([df_ratings, pd.DataFrame([rating_entry])], ignore_index=True)
        with pd.ExcelWriter(ratings_file, engine="openpyxl", mode="w") as writer:
            df_ratings.to_excel(writer, index=False)
