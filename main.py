...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type == "private" and not context.user_data.get(user_id, {}).get("session_valid") and user_id not in AUTHORIZED_USERS:
        from_group = context.application.user_data.get(f"entry_{user_id}", False)
        attempts = context.application.user_data.get(f"attempts_{user_id}", 0)

        if from_group:
            if attempts < 5:
                context.application.user_data[f"attempts_{user_id}"] = attempts + 1
            else:
