...
BASE_RATINGS = {
    "count": 4652,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
# تعديل send_suggestion لتذكرة تبدأ من 1 وتزيد تلقائياً لكل مستخدم جديد
...
async def handle_send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔹 تعديل هنا لاستخراج reply_key بشكل صحيح
    reply_key = "_".join(data_parts[2:-2])
    user_id = int(data_parts[-2])
    suggestion_id = data_parts[-1]
...
async def handle_custom_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 📌 تفعيل وضع الإدخال اليدوي ودعم الرد الجاهز
    context.user_data.setdefault(admin_id, {})
    ...
    # 🔄 ضمان جلب الرد الجاهز من السجل (كما كان يعمل سابقًا)
    ready_reply = record.get("ready_reply_text") or ""
    context.user_data[admin_id]["ready_reply_text"] = ready_reply

    msg = await query.message.reply_text(
        f"✍️ أرسل الآن الرد المخصص ليتم إرساله للمستخدم `{user_id}`:\n"
        f"أو استخدم الرد الجاهز إذا كان موجود.",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def submit_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ⭐⭐ دعم الرد الجاهز: إذا لم يكتب المشرف نصًا، نستخدم النص الجاهز من السجل
    if not reply_text:
