...
# === group_logs sheet (all chats where the bot is present) ===
try:
    df_groups = excel_data["group_logs"] if isinstance(excel_data, dict) else pd.DataFrame()
    if df_groups.empty:
        df_groups = pd.DataFrame(columns=["chat_id", "title", "type", "is_admin"])
except Exception:
    df_groups = pd.DataFrame(columns=["chat_id", "title", "type", "is_admin"])
# === group_logs sheet (all chats where the bot is present) ===
if isinstance(excel_data, dict) and "group_logs" in excel_data:
    df_groups = excel_data["group_logs"]
else:
    df_groups = pd.DataFrame(columns=["chat_id", "title", "type", "is_admin"])
