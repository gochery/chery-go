...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ تعريف الدالة المساعدة في البداية
    async def log_group_if_new(group_id, group_title):
        """📝 يحفظ معلومات المجموعات الفعالة في شيت group_logs"""
        try:
            df_groups = pd.read_excel("bot_data.xlsx", sheet_name="group_logs")
        except:
            df_groups = pd.DataFrame(columns=["group_id", "group_title"])

        if group_id not in df_groups["group_id"].values:
            df_groups = pd.concat([
                df_groups,
                pd.DataFrame([{"group_id": group_id, "group_title": group_title}])
            ], ignore_index=True)

            try:
                with pd.ExcelWriter("bot_data.xlsx", engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
                    df_groups.to_excel(writer, sheet_name="group_logs", index=False)
            except Exception as e:
                logging.error(f"[GROUP LOG] فشل حفظ المجموعات: {e}")
...
async def start_manual_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in AUTHORIZED_USERS:
        await update.message.reply_text("🚫 غير مصرح لك باستخدام هذه الميزة.")
        return

    await update.message.reply_text(
        "📝 يرجى إرسال الرسالة الدعائية الآن (نص فقط أو مع صورة/فيديو)...\n"
        "بعد إرسالها ستظهر لك قائمة المجموعات لاختيار وجهة الإرسال.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ إلغاء", callback_data="cancel_manual_broadcast")]])
    )

    # 🧠 تسجيل بداية الجلسة
    context.user_data[user_id]["broadcast_step"] = "awaiting_message"
...
async def handle_control_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    [InlineKeyboardButton("📤 إرسال تحديث لكل المجموعات", callback_data="send_update_all")],
    [InlineKeyboardButton("📣 إرسال إعلان يدوي", callback_data="start_broadcast_manual")],  
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif action == "send_update_all":
        await query.answer("📢 جاري إرسال التحديث...", show_alert=False)

        # ✅ استدعاء الدالة الموحدة للإرسال
        sent_count, failed_count = await send_update_to_all_groups(context)
    ...
    # ✅ بدء إرسال يدوي
    if action == "start_broadcast_manual":
        await context.bot.send_message(
            chat_id=user_id,
            text=(
                "📝 يرجى الآن إرسال الرسالة الدعائية (نص فقط أو مع صورة/فيديو).\n"
                "بعد إرسالها، ستظهر لك قائمة بجميع المجموعات لاختيار الجهة المستهدفة.\n\n"
                "🔒 لا تقلق، الرسالة لن تُرسل إلا بعد تأكيدك."
            ),
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ إلغاء", callback_data="cancel_manual_broadcast")]])
        )
        context.user_data[user_id]["broadcast_step"] = "awaiting_message"
...
async def handle_broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # المتابعة بعد التحقق...
    """📥 استقبال رسالة المشرف اليدوية (نص أو صورة أو فيديو) تمهيدًا للإرسال الجماعي"""
    user = update.effective_user
    user_id = user.id

    # ✅ تأكد أن المستخدم مصرح له
    if user_id not in AUTHORIZED_USERS:
        return

    # ✅ تأكد أنه في جلسة إرسال إعلان
    if not context.user_data.get(user_id, {}).get("broadcast_mode"):
        return
    ...
    if not (text or photo or video):
        await message.reply_text("❌ الرجاء إرسال نص أو صورة أو فيديو للإعلان.")
        return

    # ✅ حفظ محتوى الإعلان مؤقتًا
    context.user_data[user_id]["broadcast_content"] = {
        "text": text,
        "photo": photo,
        "video": video,
    }

    # ✅ توليد قائمة المجموعات لاحقًا بعد استقبال المحتوى
    await message.reply_text(
        "✅ تم استلام الرسالة بنجاح.\nيرجى الآن اختيار المجموعة المستهدفة أو الضغط على زر الإرسال إلى الجميع.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📋 عرض المجموعات", callback_data="choose_broadcast_group")]])
    )

async def show_group_selection_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """📋 توليد أزرار المجموعات بعد استلام الرسالة اليدوية"""
    user_id = update.effective_user.id
    if user_id not in AUTHORIZED_USERS:
        await update.message.reply_text("🚫 غير مصرح لك بإرسال إعلانات.")
        return

    try:
        df_groups = pd.read_excel("bot_data.xlsx", sheet_name="group_logs")
    except Exception as e:
        await update.message.reply_text(f"❌ تعذر تحميل قائمة المجموعات:\n{e}")
        return

    # 🧷 حفظ الرسالة المؤقتة قبل الإرسال
    message = update.message
    context.user_data[user_id]["pending_broadcast"] = {
        "text": message.text_html or None,
        "photo": message.photo[-1].file_id if message.photo else None,
        "video": message.video.file_id if message.video else None,
        "caption": message.caption_html or None
    }

    # 🧠 توليد أزرار المجموعات
    keyboard = []
    for _, row in df_groups.iterrows():
        group_title = str(row["group_title"])
        group_id = int(row["group_id"])
        keyboard.append([
            InlineKeyboardButton(group_title, callback_data=f"sendto_{group_id}")
        ])

    # ➕ زر إرسال إلى الكل
    keyboard.append([InlineKeyboardButton("📢 إرسال إلى جميع المجموعات", callback_data="sendto_all")])
    keyboard.append([InlineKeyboardButton("❌ إلغاء", callback_data="cancel_broadcast")])

    await message.reply_text(
        "📬 اختر المجموعة التي تريد إرسال الإعلان لها:",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode=constants.ParseMode.HTML
    )

    # 🧹 إيقاف وضع البث بعد توليد الأزرار
    context.user_data[user_id]["broadcast_mode"] = False
...
async def handle_broadcast_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in AUTHORIZED_USERS:
        await query.answer("🚫 لا تملك صلاحية الإرسال.", show_alert=True)
        return

    pending = context.user_data.get(user_id, {}).get("pending_broadcast")
    if not pending:
        await query.answer("⚠️ لا توجد رسالة محفوظة للإرسال.", show_alert=True)
        return

    try:
        df_groups = pd.read_excel("bot_data.xlsx", sheet_name="group_logs")
    except Exception as e:
        await query.message.reply_text(f"❌ تعذر تحميل قائمة المجموعات:\n{e}")
        return

    selected_group_ids = []

    if data == "sendto_all":
        selected_group_ids = df_groups["group_id"].astype(int).tolist()
    ...
    for gid in selected_group_ids:
        try:
            ...
            logging.warning(f"❌ فشل الإرسال إلى {gid}: {e}")
            failed += 1

    await query.message.edit_text(
        f"📦 تم إرسال الإعلان إلى {sent} مجموعة ✅ (فشل: {failed})",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ عودة", callback_data="control_back")]])
    )

    # 🧹 تنظيف الرسالة المؤقتة
    context.user_data[user_id].pop("pending_broadcast", None)

async def send_update_to_all_groups(context: ContextTypes.DEFAULT_TYPE):
    try:
        df_groups = pd.read_excel("bot_data.xlsx", sheet_name="group_logs")
    except Exception as e:
        logging.error(f"فشل تحميل قائمة المجموعات: {e}")
        return 0, 0
...
API_TOKEN = "7300700310:AAFT0DLU0-hMa2MwtwASLqGs8Lz6qR5A8Y8"
application = Application.builder().token(API_TOKEN).build()

# ✅ تسجيل الهاندلرات (نفس ترتيبك)
...
application.add_handler(CallbackQueryHandler(
    ...
    pattern="^(ctrl_maintenance_on|ctrl_maintenance_off|reload_settings|add_admin|list_admins|clear_sessions|show_stats|self_destruct|exit_control|control_back|admins_menu|restart_session|delete_admin|broadcast_update|manual_message|manual_broadcast)$"
))
application.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, handle_message))
application.add_handler(MessageHandler(filters.ALL, handle_broadcast_message))
application.add_handler(CallbackQueryHandler(handle_broadcast_selection, pattern=r"^sendto_.*$"))
...
application.add_handler(CallbackQueryHandler(handle_control_buttons, pattern=r"^(start_broadcast_manual|cancel_manual_broadcast|control_back)$"))
...
application.add_handler(CallbackQueryHandler(lambda u, c: asyncio.create_task(u.callback_query.answer("🚫 هذا الزر غير نشط حالياً.")), pattern=r"^disabled$"))
