...
def build_main_menu_keyboard(user_id: int):
    """يبني لوحة القائمة الرئيسية حسب معرف المستخدم"""
    return [
    ...
    [InlineKeyboardButton("📘 دليل المالك CHERY", callback_data=f"manual_{user_id}")],
    [InlineKeyboardButton("🏢 المتاجر ومراكز الخدمة", callback_data=f"service_{user_id}")],
    [InlineKeyboardButton("⚠️ الأعطال الشائعة وحلولها", callback_data=f"faults_{user_id}")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ تسجيل المستخدم في all_users_log
    global ALL_USERS
    ...
    if chat_id < 0:
        ...
        program_description = (
            ...
            "`التجربة الأذكى لخدمة ملاك شيري برو وإكسيد.`\n\n"

            "**⚙️ خدمات مدمجة**\n"
            "`صيانة دورية • قطع غيار • دليل المالك • مراكز خدمة ومتاجر معتمدة.`\n\n"

            "**🔧 الأعطال الشائعة وحلولها**\n"
            "`معلومات موثوقة وخطوات تساعدك على فهم المشكلة قبل زيارة الورشة.`\n\n"

            "**🛠️ مركز الدعم الفني**\n"
            "`تواصل مباشر مع فريق GO الفني للرد على استفساراتك بدقة وسرعة.`\n\n"

            "**✨ تحديثات النظام**\n"
            "`تمت إعادة بناء جميع الخدمات لتقدم لك وصولاً أسرع ونتائج أدق وتجربة أكثر احترافية.`"
        )
    ...
    msg1 = await update.message.reply_text(
        f"🧑‍💼 مرحباً {user_name}\n\n"
        "🤖 انت الان داخل *نظام الصيانة GO / CHERY* التفاعلي.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        for _, row in matches.iterrows():
            station = row.get("Station Name", "غير معروف")
            part_no = row.get("Part No", "غير معروف")
...
async def handle_manualcar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ أزرار: استعراض الدليل + رجوع للقائمة الرئيسية
    keyboard = [
...
async def handle_manualdfcar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ إذا لم يوجد PDF، نعرض رسالة تنبيه محترمة ومنسقة + زر رجوع
...
async def km_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    results = df_maintenance[
        (df_maintenance["car_type"] == car) & (df_maintenance["km_service"] == data[1])
    ]
...
async def send_cost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # حذف زر التكلفة فقط من الرسالة الأصلية
    try:
...
async def send_brochure(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    caption = (
        f"{header}"
        f"*نوع السيارة 🚗:* {result['car_type']}\n"
        f"*المسافة 📏:* {result['km_service']}\n\n"
        f"`⏳ سيتم حذف هذا الاستعلام تلقائيًا خلال 10 دقائق ({delete_time} / 🇸🇦)`"
    )

    # 🔙 زر رجوع مع رسالة البروشور
    back_keyboard = InlineKeyboardMarkup(
        [[InlineKeyboardButton("⬅️ رجوع للقائمة الرئيسية", callback_data=f"back_main_{user_id}")]]
    )
    ...
    # حذف زر عرض الملف فقط من الرسالة الأصلية
    try:
        keyboard = query.message.reply_markup.inline_keyboard
        updated_keyboard = [
            row for row in keyboard
            if not any("brochure_" in button.callback_data for button in row)
        ]
        await query.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup(updated_keyboard))
    except Exception:
        pass
...
async def _send_independent_results(update: Update, context: ContextTypes.DEFAULT_TYPE, filter_type: str):
    ...
    for _, row in results.iterrows():
        ...
        text += (
            f"\n`⏳ سيتم حذف هذا الاستعلام تلقائيًا خلال 10 دقائق ({delete_time} / 🇸🇦)`"
        )

        msg = await query.message.reply_text(text, parse_mode=constants.ParseMode.MARKDOWN)
        register_message(user_id, msg.message_id, query.message.chat_id, context)

    # ✅ زر رجوع بعد عرض النتائج
    back_keyboard = [[InlineKeyboardButton("⬅️ رجوع للقائمة الرئيسية", callback_data=f"back:main:{user_id}")]]
    back_msg = await query.message.reply_text(
        "⬅️ يمكنك العودة إلى القائمة الرئيسية في أي وقت:",
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if raw_data.startswith("back_main_"):
        ...
        keyboard = build_main_menu_keyboard(user_id)
        msg = await query.edit_message_text("اختر الخدمة المطلوبة:", reply_markup=keyboard)
        register_message(user_id, msg.message_id, query.message.chat_id, context)
    ...
    if raw_data.startswith("catpart_"):
        ...
        # باقي الأنواع الأخرى مثل parts_123 أو suggestion_123 أو faults_123
        if len(data) < 2:
    ...
    if action == "faults":
        ...
        text = (
            ...
            "`⏳ سيتم حذف هذا الاستعلام تلقائياً خلال 10 دقائق "
            f"({delete_time} / 🇸🇦)`"
        )
        ...
        for _, row in subset.iterrows():
            ...
            text += (
                f"`⏳ سيتم حذف هذا الاستعلام تلقائياً خلال 10 دقائق "
                f"({delete_time} / 🇸🇦)`"
            )
        ...
        # رسالة ختامية فيها زر رجوع
        back_keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅️ رجوع للقائمة الرئيسية", callback_data=f"back_main_{user_id}")]]
        )
        back_msg = await query.message.reply_text(
            "يمكنك العودة إلى القائمة الرئيسية أو اختيار تصنيف أعطال آخر:",
            reply_markup=back_keyboard
        )
        register_message(user_id, back_msg.message_id, query.message.chat_id, context)
    ...
    # ================== بقية الأزرار كما في كودك الحالي ==================

    if action == "parts":
        keyboard = [
            [InlineKeyboardButton("🧩 استعلام قطع الغيار الاستهلاكية", callback_data=f"consumable_{user_id}")],
            [InlineKeyboardButton("🧩 استعلام قطع غيار عام", callback_data=f"extparts_{user_id}")],
            [InlineKeyboardButton("⬅️ رجوع للقائمة الرئيسية", callback_data=f"back_main_{user_id}")]
        ]

        msg = await query.edit_message_text("اختر نوع استعلام قطع الغيار ⚙️ :", reply_markup=InlineKeyboardMarkup(keyboard))
        register_message(user_id, msg.message_id, query.message.chat_id, context)
        ...
        register_message(user_id, msg.message_id, query.message.chat_id, context)
        await log_event(update, "عرض قائمة السيارات للقطع الاستهلاكية")
        return
        ...
        keyboard = [
            [InlineKeyboardButton(car, callback_data=f"car_{car.replace(' ', '_')}_{user_id}")]
            for car in cars
        ]
        keyboard.append(
            [InlineKeyboardButton("⬅️ رجوع للقائمة الرئيسية", callback_data=f"back_main_{user_id}")]
        ...
        # 📨 مركز الدعم الفني + زر رجوع للقائمة الرئيسية
        ...
        msg = await query.edit_message_text(
            "✉️ يرجى كتابة استفساراك او ملاحظتك لفريق الدعم مع امكانية إرسال صورة أو فيديو أو ملف :",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        register_message(user_id, msg.message_id, query.message.chat_id, context)
        await log_event(update, "بدأ المستخدم إرسال استفسار")
...
async def handle_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt_block = (
        ...
        "✉️ يرجى كتابة استفسارك أو ملاحظتك مع إمكانية إرسال صورة أو فيديو أو ملف توضيحي."
    )
    delete_block = (
        f"`⏳ سيتم حذف هذه الرسالة تلقائيًا خلال 10 دقائق ({delete_time} / 🇸🇦)`"
    )
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # نحاول حذف رسالة الأزرار القديمة إن أمكن
    try:
        await query.message.delete()
    except:
        pass

    # ✅ رسالة شكر للمستخدم مع تنسيق الاسم + زر رجوع للقائمة الرئيسية
    thank_you_text = (
        f"`🧑‍💼 استفسار مسجل باسم {user_name}`\n\n"
        "🎉 شكرًا لمساهمتك معنا!\n\n"
        "✅ تم إرسال الاستفسار بنجاح إلى فريق GO الفني.\n"
        "📌 سيتم الاطلاع عليه والرد عليك في أقرب فرصة بإذن الله.\n"
    )
    ...
    register_message(user_id, msg.message_id, user_id, context)

    # يمكن تنظيف بعض البيانات المؤقتة للجلسة بعد الإرسال
...
async def handle_send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if record.get("replied_by"):
        await query.answer("🟥 تم الرد مسبقًا على هذا الاستفسار.", show_alert=True)
        return
...
async def do_broadcast_update(update: Update, context: ContextTypes.DEFAULT_TYPE, *, notify_user_id: int):
    # نص الإعلان
    now_saudi = datetime.now(timezone.utc) + timedelta(hours=3)
    msg_text = (
        "📢 <b>إعلان هام من برنامج GO</b>\n\n"
        "🚀 تم تحديث البرنامج بالكامل!\n"
        "🛠️ قوائم أسرع • نتائج أدق • واجهة أسهل\n\n"
        "🔧 صيانة دورية • 🧩 قطع الغيار • 📘 دليل المالك • 🗺️ المراكز والمتاجر\n\n"
        f"<code>🕓 وقت التحديث: {now_saudi:%Y-%m-%d %I:%M %p} 🇸🇦</code>\n\n"
        "🌟 شكراً لثقتكم — فريق <b>GO</b>"
    )
    image_path = get_update_image_path()
    targets = collect_target_chat_ids(context)
    if not targets:
        await context.bot.send_message(chat_id=notify_user_id, text="ℹ️ لا توجد مجموعات/قنوات معروفة للإرسال حالياً.")
        return
    ...
    for gid in sorted(targets):
        ...
        try:
            ...
            logging.warning(f"[broadcast_update] فشل الإرسال إلى {gid}: {e}")
            failed += 1
    await context.bot.send_message(
        chat_id=notify_user_id,
        text=f"📊 تقرير الإرسال: ✅ ناجحة {sent} • ⚠️ فاشلة {failed} • ⏭️ مُتخطّاة {skipped}"
    )
