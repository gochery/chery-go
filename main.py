...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # حساب وقت الحذف لإضافته داخل الرسالة
    delete_time = (datetime.now() + timedelta(minutes=15)).strftime("%I:%M %p")

    # ✅ رسالة شكر + رقم التذكرة + إشعار الحذف مدمج في النهاية
    thank_you_message = (
    ...
    # 👉 إرسال رسالة الشكر وإرجاع message_id
    sent_msg = await context.bot.send_message(
        chat_id=user_id,
    ...
    # 👉 جدولة حذف رسالة الشكر نفسها فقط
    context.job_queue.run_once(
        lambda _: context.bot.delete_message(chat_id=user_id, message_id=sent_msg.message_id),
        when=900,   # 15 دقيقة
