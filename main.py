...
BASE_RATINGS = {
    "count": 3991,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
async def show_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"{user_rating_line}"
        "<b>📌 الملخص العام</b>\n"
        f"👥 عدد مستخدمين GO داخل النظام: <a href=\"tg://user?id=0\">{total_users}</a>\n"
        f"🏡 عدد القروبات المرتبطة بـ GO داخل النظام: <a href=\"tg://user?id=0\">{total_groups}</a>\n"
        f"🚀 إجمالي مرات استخدام GO داخل النظام: <a href=\"tg://user?id=0\">{total_go}</a>\n\n"
        f"{rating_info}\n\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id < 0:
        ...
        program_description = (
            "**🚀 أبدا خدمتك الآن مع النسخة المطوّرة من نظام GO**\n"
            "`التجربة الأذكى لخدمة ملاك شيري / إكسيد / ام جي / جايكو / أومودا / سوايست / جيتور / BYD.`\n\n"
            "**⚙️ خدمات تفاعلية**\n"
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 👉 إرسال رسالة المستخدم (الرسالة التي سيتم حذفها بعد 15 دقيقة)
    sent_msg = await context.bot.send_message(
        chat_id=user_id,
    ...
    # 👉 إشعار الحذف بخط نحيف أسفل الرسالة — تماماً مثل بقية الخدمات
    delete_time = (datetime.now() + timedelta(minutes=15)).strftime("%I:%M %p")
    delete_notice = await context.bot.send_message(
        chat_id=user_id,
        text=f"`⏳ سيتم حذف هذه الرسالة تلقائيًا خلال 15 دقيقة ({delete_time})`",
        parse_mode=ParseMode.MARKDOWN
    )

    # 👉 جدولة الحذف الفعلي (نفس الآلية المستخدمة في جميع الخدمات)
    context.job_queue.run_once(
        lambda _: (
            context.bot.delete_message(chat_id=user_id, message_id=sent_msg.message_id),
            context.bot.delete_message(chat_id=user_id, message_id=delete_notice.message_id)
        ),
        when=900,   # 15 دقيقة
        name=f"delete_suggestion_{sent_msg.message_id}"
    )
