...
try:
    # ◆ تحميل البيانات مع دعم النسخ الاحتياطية
    excel_data = _load_excel_with_backup()

    # ◆ تحميل كل الشيتات بأمان (بدون سقوط في حال شيت ناقص)
    ...
    # ================================================================
    #  تحميل المجموعات المسجلة مسبقاً عند الإقلاع
    # ================================================================
    ...
    try:
        ...
        logging.error(f"[GROUP_LOG INIT] خطأ في تهيئة المجموعات: {e}")
        BROADCAST_GROUPS = {}

    # ================================================================
    #  استخراج قائمة السيارات الفريدة
    # ================================================================
    try:
        unique_cars = sorted(
            df_parts["Station No"]
    ...
    # ================================================================
    #  قائمة المشرفين AUTHORIZED_USERS
    # ================================================================
    try:
    ...
    # 🔥 فشل تحميل (الباك أب + الأساسي)
    logging.error(f"[DATA LOAD ERROR] فشل قراءة البيانات: {e}")
    ...
    # ================================================================
    #  تحميل الفروع branches — مهم جداً
    # ================================================================
    try:
    ...
    application.bot_data["branches"] = initial_branches  # ← مهم لقائمة الفروع
    ...
    # نرجع قيم آمنة
    df_admins = pd.DataFrame(columns=["manager_id", "username", "note"])
    df_replies = pd.DataFrame(columns=["key", "reply"])
    df_branches = pd.DataFrame()
    df_maintenance = pd.DataFrame()
    df_parts = pd.DataFrame()
    ...
    # حتى في حالة الخطأ نخليها قيمة معروفة (لازم للبوت)
    application.bot_data["branches"] = initial_branches
...
async def update_group_logs(chat_id: int, chat_title: str, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.error(f"[GROUP_LOGS] فشل دمج البيانات: {e}")
        return
