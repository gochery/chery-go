...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type in ["group", "supergroup"]:
        ...
        if from_group:
            if attempts < 5:
                context.bot_data[f"attempts_{user_id}"] = attempts + 1
            else:
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type != "private":
        ...
        # ✅ تفعيل جلسة مؤقتة + السماح بمحاولات 5 مرات في الخاص
        context.application.user_data[f"entry_{user_id}"] = True
        context.application.user_data[f"attempts_{user_id}"] = 0
