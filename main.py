...
async def show_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"{user_rating_line}"
        "<b>📌 الملخص العام</b>\n"
        f"👥 عدد مستخدمين GO داخل المجموعات: <a href=\"tg://user?id=0\">{total_users}</a>\n"
        f"🏡 عدد المجموعات المرتبطة بـ GO داخل النظام: <a href=\"tg://user?id=0\">{total_groups}</a>\n"
        f"🚀 إجمالي مرات استدعاءGO داخل المجموعات: <a href=\"tg://user?id=0\">{total_go}</a>\n\n"
        f"{rating_info}\n\n"
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔹 رسائل القطع داخل التصنيف
        ...
        # ⛔ التحقق من انتهاء الجلسة عند محاولة إرسال استفسار جديد
        # (الخيار الثاني فقط — الإغلاق عند الضغط وليس تلقائي)
        last_session = None
        user_sessions = suggestion_records.get(user_id, {})
        for sid, rec in user_sessions.items():
        ...
        if last_session:
            ...
            if last_time:
                ...
                diff_minutes = (now_time - last_time).total_seconds() / 60

                if diff_minutes > 60:
                    real_name = query.from_user.full_name
                    await query.answer(
                        f"⚠️ عزيزي {real_name}\nانتهى وقت الاتصال بمركز الدعم الفني.\nالرجاء كتابة GO لبدء جلسة جديدة.",
                        show_alert=True
                    )
                    return
