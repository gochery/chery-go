...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        for idx, row in matches.iterrows():
            ...
            if price:
                # نضيف كلمة ريال برمجياً إذا لم تكن موجودة
                price_display = price
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if raw_data.startswith("back_main_"):
        ...
        register_message(user_id, msg.message_id, query.message.chat_id, context)
        await log_event(update, f"⬅️ رجوع الى القائمة الرئيسية")
        return

    # زر الاعطال الشائعة مؤقتا تحت التطوير
    elif action == "faults":
        # 🔧 خدمة الاعطال الشائعة
        try:
            faults_df = df_faults
        except NameError:
            faults_df = pd.DataFrame()

        user_id = int(data[1]) if len(data) > 1 and data[1].isdigit() else query.from_user.id
        context.user_data.setdefault(user_id, {})
    ...
    if action == "parts":
        ...
        # 🔙 رجوع من قائمة اختيار فئة السيارة
        keyboard.append(
            [InlineKeyboardButton("⬅️ رجوع للقائمة الرئيسية", callback_data=f"back_main_{user_id}")]
        )
