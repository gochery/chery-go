...
from telegram import constants              # لا تضع هذا داخل أقواس أي import آخر
from telegram.constants import ParseMode    # اختياري؛ استخدمه إذا كنت تكتب ParseMode مباشرة

# ── Telegram.ext (سطر متعدد مغلق بشكل صحيح) ─────────────
...
try:
    ...
    # ✅ سجل كل المجموعات/القنوات التي يعمل بها البوت
    try:
        df_groups = excel_data.get("group_logs", pd.DataFrame())
        if df_groups.empty:
            df_groups = pd.DataFrame(columns=["chat_id", "title", "type", "is_admin"])
    except Exception:
        df_groups = pd.DataFrame(columns=["chat_id", "title", "type", "is_admin"])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        if (update.effective_chat and getattr(update.effective_chat, 'id', 0) < 0):
            await track_chat(update.effective_chat, context)
    except Exception:
        pass
    ...
    if chat.type == "private" and not context.user_data.get(user_id, {}).get("session_valid") and user_id not in AUTHORIZED_USERS:
        ...
        if text in ["/start", "start", "go", "/go"] and "start=go" not in text:
            ...
            alert_message = (
                ...
                "🔐 زر الانطلاق مخصص فقط لمن بدأ الجلسة من المجموعة بنفسه.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode in ["suggestion", "custom_reply"]:
        ...
        if has_text and has_media:
            ...
            await message.reply_text("📎 تم حفظ النص حاليا. يمكنك الآن إدخال وسائط أو الإرسال:", reply_markup=InlineKeyboardMarkup(buttons))
        elif has_media:
            await message.reply_text("🖼️ تم حفظ الوسائط حاليا. يمكنك الآن إدخال نص أو الإرسال:", reply_markup=InlineKeyboardMarkup(buttons))
        else:
    ...
    if (
        ...
        footer = f"<code>📸 الصور متاحة عبر التصنيفات\n⏳ الحذف التلقائي خلال 10 دقائق ({delete_time} 🇸🇦)</code>"
...
async def handle_manualcar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ نقل التحقق من التكرار هنا بعد return السابق
...
async def handle_manualdfcar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    caption = (
        f"`🧑‍💼 استعلام خاص بـ {user_name}`\n\n"
        f"📘 دليل المالك للسيارة: {car_name}\n\n"
        f"`⏳ سيتم حذف هذا الملف تلقائيًا خلال 10 دقائق ({delete_time} / 🇸🇦)`"
    )
...
async def send_part_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    caption = (
        ...
        f"`⏳ سيتم حذف هذا الاستعلام خلال 10 دقائق ({delete_time} 🇸🇦)`"
    )
...
async def km_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get(user_id, {}).get("km_used"):
        await query.answer(f"❌ مرحبا {query.from_user.full_name}، لا يمكنك فتح هذا الاستعلام مرتين بنفس الجلسة. الرجاء استخدام go من جديد.", show_alert=True)
        return
...
async def send_brochure(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get(user_id, {}).get("brochure_used"):
        await query.answer(
            f"❌ مرحبا {query.from_user.full_name}، لا يمكنك فتح هذا الاستعلام مرتين بنفس الجلسة. الرجاء استخدام go مره اخرى.", 
            show_alert=True
        )
        return
...
async def track_chat(chat, context):
    """يسجّل/يحدّث بيانات المجموعة/القناة في df_groups ويكتبها داخل bot_data.xlsx."""
    try:
        ...
        try:
            ...
            is_admin = getattr(me, "status", None) in ("administrator", "creator")
        except Exception:
            is_admin = False

        global df_groups
        import pandas as pd  # local guard
        if not df_groups.empty and (df_groups["chat_id"].astype("int64") == cid).any():
            df_groups.loc[df_groups["chat_id"].astype("int64") == cid, ["title", "type", "is_admin"]] = [title, ctype, is_admin]
        else:
            df_groups = pd.concat([
                df_groups,
                pd.DataFrame([{"chat_id": cid, "title": title, "type": ctype, "is_admin": is_admin}])
            ], ignore_index=True)

        # احفظ الورقة داخل Excel
        try:
            data_xlsx = None
            for cand in ("BOT_DATA_XLSX", "DATA_XLSX_PATH", "DATA_XLSX", "DATA_FILE", "BOT_DATA_FILE"):
                if cand in globals() and isinstance(globals()[cand], str) and globals()[cand]:
                    data_xlsx = globals()[cand]
                    break
            if not data_xlsx:
                data_xlsx = "bot_data.xlsx"

            with pd.ExcelWriter(data_xlsx, engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
                df_groups.to_excel(writer, sheet_name="group_logs", index=False)
        except Exception:
            pass
    except Exception as e:
        import logging
        logging.warning(f"[track_chat] فشل تتبّع {getattr(chat, 'id', None)}: {e}")


async def __go_broadcast_update_all__(query, context):
    """يرسل إشعار التحديث إلى كل المجموعات/القنوات التي البوت فيها مشرف + دمج الجلسات الحديثة."""
    ...
    # 1) من السجل الدائم group_logs: كل المحادثات التي البوت فيها مشرف
    group_ids = set()
    try:
        group_ids = set(df_groups.loc[df_groups["is_admin"] == True, "chat_id"].astype(int).tolist())
    except Exception:
        pass

    # 2) دمج احتياطي: أي مجموعات ظهرت حديثًا في user_sessions (جلسات حيّة)
    try:
        for sessions in user_sessions.values():
            for session in sessions:
                gid = session.get("chat_id")
                if isinstance(gid, int) and gid < 0:
                    group_ids.add(gid)
    except Exception:
        pass

    # حاول إيجاد صورة افتراضية لو كانت متوفرة
    image_path = None
    for cand in ("UPDATE_IMAGE_PATH", "BROADCAST_IMAGE_PATH", "DEFAULT_UPDATE_IMAGE", "image_path"):
        if cand in globals() and isinstance(globals()[cand], str) and globals()[cand]:
            image_path = globals()[cand]
            break
    ...
    try:
        await query.message.reply_text(
            f"📬 تم إرسال التحديث إلى {sent_count} محادثة (فشل: {failed_count})",
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ إنهاء وضع الصيانة
    elif action == "ctrl_maintenance_off":
    ...
    # ✅ إرسال إشعار بتحديث البوت (سطر واحد فقط ويُنهي الفرع)
    elif action == "broadcast_update":
        return await __go_broadcast_update_all__(query, context)

    # ✅ العودة لواجهة لوحة التحكم
    elif action == "control_back":
    ...
    # ✅ خروج من اللوحة
    elif action == "exit_control":
        await query.message.delete()
        return

    # ✅ تدمير البيانات (مقيد بصلاحية)
