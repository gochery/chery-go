...
async def parts_brand_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if brand_col:
        subset = subset[subset[brand_col].astype(str).str.strip() == brand]

    car_col_candidates = ["Station No", "car_name", "Car", "الفئة"]
    car_col = next((c for c in car_col_candidates if c in subset.columns), None)
    ...
    if not car_names:
        ...
        # مهم: لو الرسالة الأصلية صورة نستخدم reply_text
        try:
            if query.message and query.message.text:
                msg = await query.edit_message_text(
                    text,
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                car,
                callback_data=f"selectcarparts_{car.replace(' ', '_')}_{user_id}",
            )
        ]
        for car in car_names
    ]
    ...
    text = (
        f"🔧 البراند: {brand}\n\n"
        "🚗 اختر فئة السيارة لعرض القطع الاستهلاكية المتاحة:\n\n"
        f"`⏳ سيتم حذف هذا الاستعلام تلقائياً خلال 15 دقيقة ({delete_time} / 🇸🇦)`"
    )
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "faults":
        ...
        # 🔥 إرسال الأزرار فقط بدون أي نص
        back_msg = await context.bot.send_message(
            chat_id=query.message.chat_id,
            text="",   # لا يوجد نص إطلاقاً
            reply_markup=back_keyboard
        )
    ...
    # 🔹 رسائل القطع داخل التصنيف
        ...
        # 🔥 تعديل الرسالة التي فيها أزرار التصنيف، بدون إرسال رسالة جديدة
        try:
            await query.message.edit_reply_markup(back_keyboard)
        except:
            pass
...
async def handle_send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_additional:
        ...
        user_caption = (
            ...
            f"\u200F💬 *رد المشرف:*\n"
            f"```{reply_text.strip()}```\n\n"
            f"\u200F🤖 *شكرًا لثقتك بقريق الصيانة GO.*"
        )
