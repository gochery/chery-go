...
async def save_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # شكل الكول باك: ratingval_قيمة_رقمالمستخدم
    if len(parts) != 3:
    ...
    try:
        ...
        try:
            ...
            df_ratings = pd.DataFrame(columns=["user_id", "name", "rating", "timestamp", "group_name", "group_id"])
        ...
        # محاولة حذف رسالة الأزرار القديمة (لو لسه موجودة)
        try:
            if query.message:
                await context.bot.delete_message(
                    chat_id=query.message.chat_id,
        ...
        # ✅ تنظيف مود التقييم من user_data بدون ما يرمي KeyError
        ...
        await query.answer("⚠️ حدث خطأ أثناء حفظ التقييم.\n\nحاول مرة أخرى لاحقاً.", show_alert=True)
