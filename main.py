...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if raw_data.startswith("back_main_"):
        ...
        # تجهيز كيبورد القائمة الرئيسية بشكل آمن (سواء رجّعت List أو InlineKeyboardMarkup)
        kb = build_main_menu_keyboard(user_id)
        if isinstance(kb, InlineKeyboardMarkup):
            main_menu_markup = kb
        else:
        ...
        # 📌 لو الرسالة نص، نحاول تعديلها، ولو كانت صورة / ملف نرسل رسالة جديدة
        try:
            if query.message and query.message.text:
                msg = await query.edit_message_text(text_main, reply_markup=main_menu_markup)
            else:
    ...
    context.user_data.setdefault(user_id, {})
    context.user_data[user_id]["group_title"] = getattr(chat, "title", None) or "خاص"
    ...
    if action == "parts":
        ...
        suggestion_records[user_id][suggestion_id]["group_name"] = chat.title if getattr(chat, "title", None) else "خاص"
