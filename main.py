...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data[user_id].get("attempts_left", 3) <= 0 and user_id not in AUTHORIZED_USERS:
        now_saudi = datetime.now(timezone.utc) + timedelta(hours=3)
        delete_time = (now_saudi + timedelta(minutes=15)).strftime("%I:%M %p")
        msg = await update.message.reply_text(
            f"🧑‍🏫 مرحبا {user_name}\n\n"
            "🚫 لقد استنفدت جميع محاولاتك في الجلسة الحالية.\n"
            "🔄 الرجاء العودة إلى المجموعة وبدء جلسة جديدة من GO ليتم إعادة المحاولات.\n\n"
            f"⏳ سيتم حذف هذا التنبيه خلال 15 دقيقة ({delete_time} / 🇸🇦)",
            parse_mode=constants.ParseMode.MARKDOWN
        )
        register_message(user_id, msg.message_id, chat_id, context)
        return
