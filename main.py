...
async def create_excel_backup(
    ...
    وترسل (اختياريًا) إلى شات النسخ الاحتياطي المحدد في TG_BACKUP_CHAT_ID
    """
    ...
    try:
        ...
        # إشعار الشخص الذي ضغط زر النسخ الاحتياطي (إن وجد)
        if context and notify_chat_id:
            try:
                await context.bot.send_message(
                    chat_id=notify_chat_id,
        ...
        # إرسال النسخة إلى قناة / قروب النسخ الاحتياطي إن تم ضبطه في المتغير البيئي
        backup_chat_id = 0
        try:
            env_val = os.getenv("TG_BACKUP_CHAT_ID", "").strip()
            if env_val:
                backup_chat_id = int(env_val)
        except Exception:
            backup_chat_id = 0

        if context and backup_chat_id:
            try:
                ...
                logging.error(f"[BACKUP] ❌ فشل إرسال النسخة الاحتياطية إلى TG_BACKUP_CHAT_ID: {e}")
...
BASE_RATINGS = {
    "count": 1733,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.bot_data.get("maintenance_mode"):
        ...
        # محاولة التعامل مع اختلاف حالة الأحرف في اسم ملف الصورة بين الأنظمة
        photo_path = "go-ss.jpg"
        if not os.path.exists(photo_path):
            if os.path.exists("go-ss.JPG"):
                photo_path = "go-ss.JPG"
            else:
                photo_path = None
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global df_admins, AUTHORIZED_USERS
    query = update.callback_query
    action = query.data
    user_id = query.from_user.id
...
application.add_handler(CallbackQueryHandler(
    ...
    pattern="^(ctrl_maintenance_on|ctrl_maintenance_off|reload_settings|clear_sessions|self_destruct|exit_control|control_back|admins_menu|list_admins|add_admin|delete_admin|restart_session|ctrl_backup)$"
))
...
async def on_startup():
    ...
    # ✅ تفعيل JobQueue (تنظيف الجلسات القديمة + النسخ الاحتياطي اليومي)
