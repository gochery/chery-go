...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ❌ منع استقبال أي استفسار نصّي بعد أول تذكرة بدون استخدام زر "إرسال استفسار آخر"
    if chat.type == "private":
        suggestion_count = context.user_data.get(user_id, {}).get("suggestion_count", 0)
        waiting_for_button = context.user_data.get(user_id, {}).get("active_suggestion_id")

        # إذا أنهى تذكرته الأولى ولم يبدأ جلسة جديدة عبر الزر
        if suggestion_count >= 1 and waiting_for_button is None:
            # تجاهل الرسالة وعدم الرد
            return
