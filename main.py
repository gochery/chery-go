...
BASE_RATINGS = {
    "count": 4265,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type == "private" and not context.user_data.get(user_id, {}).get("session_valid") and user_id not in AUTHORIZED_USERS:
        ...
        if text in ["/start", "start", "go", "/go"] and "start=go" not in text:
            ...
            alert_message = (
                ...
                "🔐 زر الانطلاق يستعمل لمرة واحدة وهو مخصص فقط لمن بدأ الجلسة من المجموعة بنفسه.\n"
    ...
    context.user_data[user_id].setdefault("attempts_left", 3)  # ← عداد المحاولات
    ...
    if chat_id < 0:
        ...
        return  # ينهي فرع المجموعة

    # ------------------------------------------------------------------------
    # فرع الخاص بعد تعديل منطق الجلسة
    # ------------------------------------------------------------------------
    context.user_data.setdefault(user_id, {})
    ...
    # إعادة ضبط الجلسة الحالية لتكون صالحة للخاص فقط إذا غير صالحة
    if not context.user_data[user_id].get("session_valid", False):
        context.user_data[user_id]["session_valid"] = True
        context.user_data[user_id]["attempts_left"] = 3
        logging.info(f"[SESSION RESET] تم إعادة ضبط جلسة الدعم للمستخدم {user_name} بعد العودة من المجموعة")

    # تأكيد وجود عداد المحاولات
    if "attempts_left" not in context.user_data[user_id]:
        context.user_data[user_id]["attempts_left"] = 3
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type != "private":
        # حفظ بيانات القروب للمستخدم لاستخدامها عند الانتقال للخاص
        context.bot_data[user_id] = {
            "group_title": chat.title or "غير معروف",
            "group_id": chat.id,
            "user_name": user_name
        ...
        # ✅ أرسل بانر GO / زر الانطلاق
        await start(update, context)

        # ✅ تحديث group_logs في الخلفية
        try:
    ...
    # --------------------------------------------------------------------
    # ✨ الإضافة الجديدة التي طلبتها — دمج كامل بدون تغيير أي شيء آخر
    # --------------------------------------------------------------------
    if chat.type == "private" and user_id not in AUTHORIZED_USERS:
        user_data = context.user_data.setdefault(user_id, {})

        # إذا انتهت المحاولات القديمة أو الجلسة غير صالحة
        if not user_data.get("session_valid", True) or user_data.get("attempts_left", 3) <= 0:
            # إعادة ضبط الجلسة تلقائياً
            user_data["session_valid"] = True
            user_data["attempts_left"] = 3
            logging.info(f"[SESSION AUTO-RESET] إعادة ضبط جلسة الدعم للمستخدم {user_name} تلقائياً بعد انتهاء المحاولات أو العودة من المجموعة")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ==========================================================
    #  ✨✨  من هنا أضفت منطق الدعم الفني (المحاولات الثلاث)  ✨✨
    # ==========================================================
    ...
    # 🔥 إذا كان المستخدم في وضع "suggestion" (جلسة دعم فني)
    if mode == "suggestion" and chat.type == "private":

        # ⚠️ التحقق من عدد المحاولات
        attempts = context.user_data[actual_user_id].get("support_attempts", 0)

        # إذا تجاوز 3 محاولات → مربع تنبيه + إيقاف الجلسة
        if attempts >= 3:
            await message.reply_text(
                f"⚠️ عذراً {user_name}، لقد استنفدت عدد الاستفسارات المسموح بها لهذه الجلسة.\n\n"
                "🎯 ابدأ جلسة جديدة عن طريق كتابة (go) في المجموعة.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("حسناً", callback_data="none")]])
            )
            context.user_data[actual_user_id]["session_valid"] = False
            return

        # ✨ هنا الزيادة الفعلية للمحاولات (عند كتابة نص فقط)
        if message.text:
            context.user_data[actual_user_id]["support_attempts"] = attempts + 1

        # ثم نكمل تنفيذ المنطق الطبيعي كما هو
        # (لا نلمس أي شيء هنا)
        # ==========================================================

    # ==========================================================
    # ✨ نهاية تعديل الدعم الفني — الكود الأصلي يستمر طبيعيًا ✨
    # ==========================================================

    # بقية منطق suggestion / custom_reply كما هو 100%
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ❌ لم يعد يتم زيادة أو حساب المحاولات هنا داخل callback
    attempts = user_data.get("support_attempts", 0)

    # إذا كانت الجلسة منتهية فعلاً
    if attempts >= 3:
        await query.answer(
            f"⚠️ عذراً {user_name}، لقد استنفدت جميع الاستفسارات لهذه الجلسة.\n"
            "سيتم إعادة الاتصال تلقائياً عند العودة وكتابة (go) من المجموعة.",
            show_alert=True
        )
        user_data["session_valid"] = False
        logging.info(f"[SESSION END] انتهت جلسة الدعم للمستخدم {user_name} بعد 3 محاولات")
        return

    # ❗ لا زيادة لمحاولات الدعم هنا
    # user_data["support_attempts"] = attempts + 1   ← تمت إزالتها نهائياً
    ...
    # إرسال الاستفسار للمشرفين (بدون تغيير)
...
async def submit_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ⭐⭐ إضافة دعم الرد الجاهز بدون أي تعديل آخر ⭐⭐
    if not reply_text:
