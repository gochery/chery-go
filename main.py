...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🎯 عند بدء جلسة GO جديدة يتم إعادة تفعيل مركز الدعم الفني
    context.user_data[user_id]["last_support_time"] = datetime.now()
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔹 رسائل القطع داخل التصنيف
        ...
        # ⛔ صلاحية زر "إرسال استفسار آخر" لمدة ساعة فقط من آخر استخدام
        last_support = context.user_data[user_id].get("last_support_time")
        if last_support:
            diff_minutes = (datetime.now() - last_support).total_seconds() / 60
            if diff_minutes > 60:
                real_name = query.from_user.full_name
                await query.answer(
                    f"⚠️ عزيزي {real_name}\nانتهت صلاحية جلسة الدعم الفني.\nالرجاء العودة إلى المجموعة وكتابة GO لبدء جلسة دعم جديدة.",
                    show_alert=True
                )
                return
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data[user_id]["active_suggestion_id"] = None
    # تحديث وقت آخر استخدام للدعم الفني
    context.user_data[user_id]["last_support_time"] = datetime.now()
