...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ السماح بالدخول من الخاص لمدة محدودة بعد الدخول من المجموعة
    if user_id in AUTHORIZED_USERS:
        context.user_data.setdefault(user_id, {})["session_valid"] = True

    if chat.type in ["group", "supergroup"]:
        context.bot_data[f"entry_{user_id}"] = True
        context.bot_data[f"attempts_{user_id}"] = 0
        # 👇 نسمح باستمرار التنفيذ لعرض رسالة الترحيب لاحقًا
    elif chat.type == "private" and not context.user_data.get(user_id, {}).get("session_valid") and user_id not in AUTHORIZED_USERS:
        from_group = context.bot_data.get(f"entry_{user_id}", False)
        attempts = context.bot_data.get(f"attempts_{user_id}", 0)

        if from_group:
            if attempts < 5:
                context.application._user_data[user_id]["attempts"] = attempts + 1
                # ✅ عرض عدد المحاولات
                remaining = attempts + 1
                counter_text = f"📊 المحاولة {remaining} من 5"
            else:
                ...
                msg = await context.bot.send_message(
                    ...
                    text=(
                        f"`🧑‍🏫 مرحبا {user_name}`\n\n"
                        "🚫 لقد استنفدت جميع محاولات الدخول من الخاص.\n"
                        "🔁 يرجى العودة إلى المجموعة وكتابة الأمر go لإعادة التفعيل.\n\n"
            ...
            if text in ["/start", "start", "go", "/go"] and "start=go" not in text:
                ...
                alert_message = (
                    ...
                    "🔐 زر الانطلاق يستحدم لمرة واحدة فقط وهو مخصص لمن بدأ الجلسة من المجموعة بنفسه.\n"
...
async def handle_go_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type != "private":
        ...
        # ✅ تفعيل جلسة مؤقتة + السماح بـ 5 محاولات في الخاص
        if not hasattr(context.application, "_user_data"):
            context.application._user_data = {}

        if user_id not in context.application._user_data:
            context.application._user_data[user_id] = {}

        context.application._user_data[user_id]["entry"] = True
        context.application._user_data[user_id]["attempts"] = 0

        context.user_data[user_id] = context.user_data.get(user_id, {})
        context.user_data[user_id]["session_valid"] = True

        # ✅ تنظيف مفاتيح الصور المفتوحة سابقًا
        keys_to_remove = [key for key in context.user_data[user_id] if key.startswith("image_opened_")]
        for key in keys_to_remove:
            del context.user_data[user_id][key]

    # ✅ منع المتطفلين في الخاص بدون جلسة صالحة
    if chat.type == "private" and (
        ...
        user_block = f"`\U0001f9d1‍\U0001f3eb مرحبا {user_name}`"
        alert_message = (
            "\U0001f4e3 يسعدنا اهتمامك بخدمات برنامج GO!\n\n"
            "❌ لا يمكنك بدء الخدمة مباشرة من الخاص.\n"
            "🔐 الرجاء الدخول إلى المجموعة وكتابة go بنفسك.\n\n"
            "[\U0001f465 اضغط هنا للانضمام إلى مجموعة CHERY](https://t.me/CHERYKSA_group)"
        )
        delete_block = f"`⏳ سيتم حذف هذا التنبيه تلقائيًا خلال 5 دقائق ({delete_time} / 🇦🇪)`"
