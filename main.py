...
async def start_suggestion_session(user_id, context):
    ...
    ticket_no = SUGGESTION_TICKET_COUNTER  # ← رقم التذكرة هنا
    ...
    # ✅ سجل الاقتراح — هذا هو المكان الصحيح لوضع رقم التذكرة
    suggestion_records.setdefault(user_id, {})
    suggestion_records[user_id][suggestion_id] = {
        "ticket_no": ticket_no,      # ← المكان الصحيح والمطلوب
        "text": None,
        "media": None,
        "submitted": False,
        "admin_messages": {},
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إنشاء تذكرة جديدة لكل استفسار متسلسل
    suggestion_records.setdefault(user_id, {})
    ticket_no = len(suggestion_records[user_id]) + 1      # رقم متسلسل 1-2-3
    suggestion_id = f"{user_id}_{ticket_no}"             # معرف فريد لكل استفسار
    ...
    logging.info(f"[تأكيد المجموعة] المستخدم: {user_id} | المجموعة: {record['group_name']} | ID: {record['group_id']}")

    header = (
        f"👤 الاسم: {record['user_name']}\n"
        f"🆔 رقم المستخدم: <code>{user_id}</code>\n"
        f"🏘️ المجموعة: {record['group_name']}\n"
        f"🔢 رقم المجموعة: <code>{record['group_id']}</code>\n"
        f"🎫 رقم التذكرة: <code>#{record['ticket_no']}</code>\n"
        "╰─────────╯"
    )
    ...
    # إرسال الاستفسار لكل مشرف دون مسح الردود السابقة
    for admin_id in AUTHORIZED_USERS:
        try:
            ...
            if media:
                ...
                if mtype == "photo":
                    sent = await context.bot.send_photo(admin_id, fid, caption=full_caption,
                                                        parse_mode=ParseMode.HTML, reply_markup=keyboard)
                elif mtype == "video":
                    sent = await context.bot.send_video(admin_id, fid, caption=full_caption,
                                                        parse_mode=ParseMode.HTML, reply_markup=keyboard)
                elif mtype == "document":
                    sent = await context.bot.send_document(admin_id, fid, caption=full_caption,
                                                           parse_mode=ParseMode.HTML, reply_markup=keyboard)
                elif mtype == "voice":
                    sent = await context.bot.send_voice(admin_id, fid, caption=full_caption,
                                                        parse_mode=ParseMode.HTML, reply_markup=keyboard)
            else:
                suggestion_block = f"\n\n📝 <b>الاستفسار الوارد:</b>\n<code>{text}</code>" if text else ""
                full_caption += suggestion_block
                sent = await context.bot.send_message(admin_id, text=full_caption,
                                                      parse_mode=ParseMode.HTML, reply_markup=keyboard)
    ...
    # رسالة شكر + رقم التذكرة + ملخص الاستفسار
    thank_you_message = (
        f"🧑‍💼 استفسار دعم فني خاص بـ {record['user_name']}\n\n"
        "🎉 شكرًا لثقتك بفريق الصيانة والدعم الفني GO!\n"
        f"🎫 رقم تذكرتك: #{record['ticket_no']}\n\n"
    )

    if text:
        thank_you_message += f"ملخص استفسارك:\n{text}\n\n"

    thank_you_message += "✅ تم إرسال الاستفسار بنجاح إلى فريق الدعم الفني .\n" \
    ...
    # تنظيف بيانات الكتابة بعد الإرسال
    for key in ["action", "compose_mode", "compose_text", "compose_media"]:
