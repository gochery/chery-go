...
async def handle_suggestion_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ بعد أول رد: ممنوع على غير نفس المشرف (لكن نفس المشرف مسموح يرسل رد إضافي لاحقًا)
    if record.get("replied_by") and record.get("caption") and record.get("replied_by") != admin_name:
        await query.answer(
