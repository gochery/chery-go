...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg1 = await update.message.reply_text(
        f"🧑‍💼 مرحباً {user_name}\n\n"
        "🤖 انت الان داخل *نظام الصيانة GO / * التفاعلي.\n"
        "💡 يمكنك الآن بدء رحلتك الذكية معنا في خدمات الصيانة وقطع الغيار والمزيد من المعلومات في مكان واحد.\n\n"
        f"⏳ سيتم حذف هذه الرسالة تلقائيًا خلال 10 دقيقة ({delete_time} / 🇸🇦)",
        parse_mode=constants.ParseMode.MARKDOWN
    )
...
async def send_cost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if brand_raw:
        ...
        # جايكو
        elif "جايكو" in brand_raw:
            dealer_key = "جايكو"

        # أومودا
        elif "أومودا" in brand_raw or "OMODA" in upper:
            dealer_key = "أومودا"

        # جيتور
        elif "جيتور" in brand_raw or "JETOUR" in upper:
            dealer_key = "جيتور"

        # سوايست / سواسيت (اختلاف كتابة)
        elif "سوايست" in brand_raw or "سواسيت" in brand_raw:
            dealer_key = "سواسيت"

        # شيري / اكسيد / سنابل → كلهم سنابل الحديثة
        elif (
            "شيري" in brand_raw
            or "اكسيد" in brand_raw
            or "سنابل" in brand_raw
            or "CHERY" in upper
            or "EXEED" in upper
        ):
            dealer_key = "سنابل الحديثة"

        # أي اسم براند آخر نستخدمه كما هو (وتضيفه لاحقًا في BRAND_CONTACTS لو حبيت)
        if not dealer_key:
            dealer_key = brand_raw

    # لو ما في براند أصلاً (صفوف قديمة) نخليها سنابل الحديثة افتراضيًا
    if not dealer_key:
        dealer_key = "سنابل الحديثة"
    ...
    if PLACEHOLDER_TEXT in maintenance_action:
        ...
        # ✳️ الحالة العادية: عرض تكلفة الصيانة + بيانات الشركة إن وجدت
        caption = (
            f"`🧑‍💻 استعلام خاص بـ {user_name}`\n"
    ...
    try:
        ...
        updated_keyboard = [
            ...
            if not any("cost_" in button.callback_data for button in row)
        ]
...
async def send_brochure(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔙 أزرار الرجوع مع رسالة البروشور
    back_keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("⬅️ رجوع لقائمة مسافات الصيانة", callback_data=f"car_{safe_car}_{user_id}")],
            [InlineKeyboardButton("⬅️ رجوع للقائمة الرئيسية", callback_data=f"back_main_{user_id}")],
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔹 رسالة ختامية فيها أزرار رجوع
        ...
        prompt_block = (
            ...
            "✉️ يرجى كتابة استفسارك أو ملاحظتك مع إمكانية إرسال صورة أو فيديو أو ملف وسيتم الرد عليك من قبل فريق GO للدعم الفني خلال هذي المحادثة.\n\n"
...
async def handle_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt_block = (
        "💬 أهلاً بك في *مركز الدعم الفني* لبرنامج GO.\n\n"
        "✉️ يرجى كتابة استفسارك أو ملاحظتك مع إمكانية إرسال صورة أو فيديو أو ملف وسينم الرد عليك من قبل فريق GO للدعم الفني خلال هذي المحادثة."
    )
    delete_block = (
        f"`⏳ سيتم حذف هذه الرسالة تلقائيًا خلال 15 دقيقة ({delete_time} / 🇸🇦)`"
    )
...
async def handle_control_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "broadcast_update":
        ...
        message_text = (
            ...
            "فريق برنامج <b>GO</b> لخدمات الصيانة والاستشارات الفنية"
        )
...
async def webhook_handler(request: Request):
    ...
    msg = f"[WEBHOOK] وصل تحديث جديد من تيليجرام: keys={list(json_data.keys())}"
    logging.info(msg)
    print(msg)  # 👈 هذا يظهر أكيد في Logs برندر
...
async def on_startup():
    ...
    # 🔄 بناء رابط الويب هوك الصحيح مع /webhook في النهاية
    base_url = os.getenv("RENDER_EXTERNAL_URL") or "https://chery-go.onrender.com"
    ...
    print(f"🔗 Webhook set: {response.status_code} -> {webhook_url}")
