            ...
            if media_list:
                try:
                    ...
                    if non_voice_media:
                        ...
                        # نحدد وسيط رئيسي واحد يكون معه النص (كابتشن)
                        primary_media = non_voice_media[0]
                        extra_media = non_voice_media[1:]

                        first_message = None

                        # 1) نرسل الوسائط الإضافية بدون كابتشن (كل واحدة رسالة مستقلة)
                        for m in extra_media:
                            ...
                            if mtype == "photo":
                                msg = await context.bot.send_photo(chat_id, fid)
                            elif mtype == "video":
                                msg = await context.bot.send_video(chat_id, fid)
                            elif mtype == "document":
                                msg = await context.bot.send_document(chat_id, fid)
                            else:
                                continue  # أنواع أخرى نتجاهلها هنا

                            if first_message is None:
                                first_message = msg

                        # 2) نرسل الوسيط الرئيسي مع نص التوصية ككابتشن
                        pm_type = primary_media.get("type")
                        pm_fid = primary_media.get("file_id")
                        text_msg = None

                        if pm_type == "photo":
                            text_msg = await context.bot.send_photo(
                                chat_id,
                                pm_fid,
                            ...
                            text_msg = await context.bot.send_video(
                                chat_id,
                                pm_fid,
                                caption=html_text or "",
                                parse_mode=constants.ParseMode.HTML,
                            )
                        elif pm_type == "document":
                            text_msg = await context.bot.send_document(
                                chat_id,
                                pm_fid,
                                caption=html_text or "",
                                parse_mode=constants.ParseMode.HTML,
                            ...
                            # احتياط: لو نوع غير متوقع نرسل نص فقط
                        ...
                        # 3) بعدها نرسل أي voice بدون كابتشن
                        ...
                        # الرسالة اللي بنثبتها (لو طلب التثبيت): نفضّل رسالة النص (الوسيط الرئيسي)
                        sent_msg = text_msg or first_message
