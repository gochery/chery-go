...
BASE_RATINGS = {
    "count": 4694,   # 👈 عدّل هذا الرقم: عدد المقيمين الافتراضي القديم
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if attempts >= 3:
        await query.answer(
            "انتهت الاستفسارات في هذه الجلسة. أغلق الاتصال من زر الالغاء واستخدم GO بالمجموعة للعودة لمركز الدعم مرة أخرى.",
            show_alert=True
        )
        user_data.pop("action", None)
        return
    ...
    # كل استفسار جديد → تذكرة جديدة مستقلة
    ...
    # أخذ النص والوسائط من user_data مباشرة
    text = user_data.get("compose_text", "")
    media = user_data.get("compose_media")

    # fallback من last_preview_id إذا فارغ
    old_id = user_data.get("last_preview_id")
    if old_id:
        prev = suggestion_records.get(user_id, {}).get(old_id)
        if prev:
            if not text:
                text = prev.get("text", "")
            if not media:
                media = prev.get("media", None)
    ...
    await context.bot.send_message(chat_id=user_id, text=thank_you_message, parse_mode=ParseMode.MARKDOWN, reply_markup=back_keyboard)
