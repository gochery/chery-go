...
async def handle_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        await update.message.reply_text("⚠️ لا يمكنك إرسال ايستفسار جديد قبل الانتهاء من الحالي.")
        return
    ...
    await update.message.reply_text("✉️ يرجى كتابة استفسارك او ملاحظة مع امكانية إرسال صورة أو فيديو أو ملف :")
    await log_event(update, "بدأ المستخدم إرسال استفسار أو ملاحظة")
...
async def send_suggestion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📝 الرد على الاستفسار الواردة", callback_data=f"reply_{user_id}_{suggestion_id}")]
    ])
    ...
    await context.bot.send_message(
        chat_id=user_id,
        text="🎉 شكرًا لمساهمتك معنا!\n\n✅ تم إرسال الاستفسار بنجاح إلى فريق GO.\n\n📌 لمراجعته وتقيمة وسيتم الرد عليك  .",
