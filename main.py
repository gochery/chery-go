...
# ✅ نقاشات داخلية لفريق GO بين المشرفين
team_threads: dict[int, dict] = {}
...
def _patched_set_application(self, application):
    """
    استبدال weakref.ref(application) بـ lambda ترجع نفس التطبيق
    بحيث يبقى السلوك كما هو لكن بدون weakref
    """
    self._application = lambda: application

# تطبيق الترقيع قبل إنشاء الـ Application
...
# ✅ تهيئة المتغيرات
df_admins = df_replies = df_branches = df_maintenance = df_parts = df_manual = df_independent = df_faults = pd.DataFrame()
ALL_USERS = set()
user_sessions = {}

# قائمة السيارات المستخدمة في استعلام قطع الغيار الاستهلاكية
unique_cars = []

# 🧩 رسالة الطرازات غير الجاهزة (تستخدم مع كل البراندات الجديدة)
...
# 🆕 خريطة ربط البراندات بوكيلها
DEALER_FOR_BRAND = {
    "شيري / إكسيد": "سنابل الحديثة",
    "شيري": "سنابل الحديثة",
    "إكسيد": "سنابل الحديثة",
    "CHERY": "سنابل الحديثة",
    "EXEED": "سنابل الحديثة",
    # لو عندك صيغة ثانية لنفس البراند أضفها هنا
}
...
def _update_all_users_log_sync():
    """
    حفظ all_users_log في ملف Excel بشكل متزامن (يُستدعى من ثريد مستقل)
    """
    global ALL_USERS
    try:
        ...
        logging.error(f"[SAVE USERS] فشل حفظ المستخدمين في Excel (background): {e}")


async def update_all_users_log_async():
    """
    غلاف async لتحديث all_users_log في الخلفية بدون تعطيل event loop
    """
    try:
...
def _update_go_stats_sync():
    """
    تحديث عداد استخدام go في bot_stats (يعمل داخل ثريد مستقل)
    """
    try:
    ...
    if "total_go_uses" in stats_df.get("key", []):
        stats_df.loc[stats_df["key"] == "total_go_uses", "value"] += 1
    else:
        stats_df = pd.concat(
            [stats_df, pd.DataFrame([{"key": "total_go_uses", "value": 1}])],
            ignore_index=True
        )
    ...
    try:
        ...
        logging.error(f"[SAVE STATS] فشل حفظ عدد /go إلى Excel (background): {e}")


async def update_go_stats_async():
    """
    غلاف async لتحديث عداد /go في الخلفية
    """
    try:
        loop = asyncio.get_running_loop()
...
def _write_health_log_sync():
    """
    كتابة health_log كل فترة في شيت مستقل لمتابعة حالة البوت
    """
    global ALL_USERS

    # قراءة health_log الحالي إن وجد
    try:
        health_df = pd.read_excel("bot_data.xlsx", sheet_name="health_log")
    except Exception:
        health_df = pd.DataFrame(columns=["timestamp", "total_users", "total_go_uses"])

    # قراءة total_go_uses من bot_stats (إن وجد)
    total_go_uses = 0
    try:
        stats_df = pd.read_excel("bot_data.xlsx", sheet_name="bot_stats")
        mask = stats_df["key"] == "total_go_uses"
        if mask.any():
            total_go_uses = int(stats_df.loc[mask, "value"].iloc[0])
    except Exception:
        pass
    ...
    new_row = {
        ...
        "total_go_uses": total_go_uses,
    }

    health_df = pd.concat(
        [health_df, pd.DataFrame([new_row])],
        ignore_index=True
    )
    ...
    try:
        with pd.ExcelWriter(
            ...
            health_df.to_excel(writer, sheet_name="health_log", index=False)
    except Exception as e:
        logging.error(f"[HEALTH LOG] فشل حفظ health_log: {e}")


async def health_log_job(context: ContextTypes.DEFAULT_TYPE):
    """
    جوب دورية لتسجيل نبض البوت في health_log بدون تعطيل أي خدمة
    """
    try:
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, _write_health_log_sync)
    except Exception as e:
        logging.error(f"[HEALTH LOG] فشل تشغيل health_log في الخلفية: {e}")
...
def build_main_menu_keyboard(user_id: int) -> InlineKeyboardMarkup:
    """لوحة الخدمات الرئيسية في الخاص"""
    keyboard: List[List[InlineKeyboardButton]] = [
        [InlineKeyboardButton("🔧 صيانة دورية",         callback_data=f"maintenance_{user_id}")],
        [InlineKeyboardButton("🧩 استعلام قطع غيار",     callback_data=f"parts_{user_id}")],
        [InlineKeyboardButton("📘 دليل المالك",         callback_data=f"external_{user_id}")],
        [InlineKeyboardButton("🏥 مراكز الخدمة والمتاجر", callback_data=f"service_{user_id}")],
        [InlineKeyboardButton("⚡ الأعطال الشائعة والحلول", callback_data=f"faults_{user_id}")],
        [InlineKeyboardButton("🛠️ مركز الدعم الفني",     callback_data=f"suggestion_{user_id}")],
        [InlineKeyboardButton("⭐ قيّم تجربة GO",        callback_data=f"rate_{user_id}")],
    ]

    # ✅ مميزات إضافية للمشرفين فقط
    if user_id in AUTHORIZED_USERS:
        # زر إرسال توصية فنية لكل المجموعات
        keyboard.insert(
            -1,
            [InlineKeyboardButton("📡 إرسال توصية فنية", callback_data="send_reco")]
        )
        # زر نقاشات فريق GO الداخلية (موجود مسبقاً)
        keyboard.insert(
            -1,
...
def _update_go_stats_sync():
    """
    تحديث عداد استخدام go بشكل متزامن (يعمل داخل ثريد مستقل)
    """
    try:
        stats_df = pd.read_excel("bot_data.xlsx", sheet_name="bot_stats")
    except Exception:
...
async def update_go_stats_async():
    """
    غلاف async لتحديث إحصائيات go في الخلفية بدون ما يعلق event loop
    """
    try:
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, _update_go_stats_sync)
    except Exception as e:
        logging.error(f"[SAVE STATS] فشل تشغيل تحديث /go في الخلفية: {e}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ إذا النداء من مجموعة: نرسل بانر الترحيب مع زر الانطلاق ونرجع
    if chat_id < 0:
        ...
        # 👤 اسم المستخدم — خط نحيف رمادي
        user_block = f"`🧑‍💼 مرحباً {user_name}`"

        # عنوان + وصف كل خدمة (مع تحديث البراندات)
        program_description = (
            ...
            "**🛠️ مركز الدعم الفني**\n"
            "`تواصل مباشر مع فريق GO للدعم والاستشارات الفنية.`\n\n"
        )
        # ⏳ تنبيه الحذف — خط نحيف رمادي
        delete_block = f"`⏳ سيتم حذف هذا المنشور خلال 90 ثانية ({delete_time} / 🇸🇦)`"

        full_caption = (
           f"{user_block}\n\n"
           f"{program_description}"
           "💡 اضغط الزر بالأسفل للانتقال إلى الخدمة عبر GO:\n"
           f"{delete_block}"
        )
        ...
        try:
            ...
            logging.error(f"فشل في إرسال الترحيب بالفيديو/الصورة: {e}")

        return

    # ✅ هنا نكون في الخاص بعد الضغط على زر الانطلاق من المجموعة
    ...
    msg2 = await update.message.reply_text(
        ...
        reply_markup=keyboard  # ✅ نمرر الكيبورد مباشرة
    )
    ...
    # جلسة الانطلاق تستخدم لمرة واحدة فقط
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        results_header = f"<code>📌 نتائج البحث عن: {part_name_safe}</code>\n\n"
        ...
        for _, row in matches.iterrows():
            ...
            line = f"🔹 <b>{station}</b> — <code>{part_no}</code>"
            if price:
                price_disp = html.escape(str(price))
                if "ريال" not in price_disp and "SAR" not in price_disp.upper():
                    price_disp = f"{price_disp} ريال"
                line += f" — 💰 <b>{price_disp}</b>"
            lines.append(line)
        ...
        text = header + results_header + body + footer

        # 🔘 الأزرار أسفل نتيجة البحث
        keyboard_rows = []

        # زر عرض القطع المصنفة لنفس السيارة
        keyboard_rows.append(
            [InlineKeyboardButton("🗂 عرض القطع المصنفة", callback_data=f"consumable_{user_id}")]
        )

        # زر رجوع لاختيار سيارة اخرى ضمن نفس البراند (إن وجد)
...
async def send_cost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if brand_raw:
        dealer_key = DEALER_FOR_BRAND.get(brand_raw, brand_raw)
    else:
...
async def parts_brand_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cars = sorted([c for c in cars if c])
...
async def handle_branch_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🧹 حذف زرّي "📍 مواقع فروع شركة شيري" و "🔧 المتاجر ومراكز الصيانة المستقلة" من الرسالة القديمة
    try:
        keyboard = query.message.reply_markup.inline_keyboard
        updated_keyboard = [
            row for row in keyboard
    ...
    # ✅ إضافة زر "المتاجر والمراكز المستقلة" أسفل قائمة الفروع
    keyboard_rows.append(
        [InlineKeyboardButton("🔧 المتاجر ومراكز الصيانة المستقلة", callback_data=f"independent_{user_id}")]
    )
    # ✅ زر رجوع للقائمة الرئيسية أسفل القائمة
    keyboard_rows.append(
        [InlineKeyboardButton("⬅️ رجوع للقائمة الرئيسية", callback_data=f"back:main:{user_id}")]
    )
...
async def _send_independent_results(update: Update, context: ContextTypes.DEFAULT_TYPE, filter_type: str):
    ...
    for _, row in results.iterrows():
        ...
        location = row.get("location", "غير متوفر")
        notes = row.get("notes", "")
    ...
    try:
        ...
        for aid in AUTHORIZED_USERS:
            try:
                ...
                logging.warning(f"[SUGGESTION_NOTIFY] فشل إرسال إشعار للمشرف {aid}: {e}")

        await query.answer("✅ تم إرسال الرد للمستخدم.", show_alert=True)

    except Exception as e:
        logging.error(f"[HANDLE_SEND_REPLY] فشل في إرسال الرد للمستخدم {user_id}: {e}")
        await query.answer("❌ حدث خطأ أثناء إرسال الرد.", show_alert=True)
...
async def submit_admin_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔁 عدّاد الردود هنا أيضًا (يرتبط بنفس reply_count المستخدم في الردود الجاهزة)
    reply_count = int(record.get("reply_count", 0) or 0)
    is_additional = reply_count >= 1
    reply_count += 1
    record["reply_count"] = reply_count
    ...
    try:
        ...
        # حذف رسالة قائمة الخيارات من الخاص
        try:
            await query.message.delete()
        except:
            pass

        # تنظيف منيو الرد لو كان محفوظ
        if "reply_menu_chat" in record and "reply_menu_id" in record:
            for aid in AUTHORIZED_USERS:
                try:
                    await context.bot.delete_message(record["reply_menu_chat"], record["reply_menu_id"])
        ...
        # ✅ إرسال إشعار للمشرفين مع أزرار النقاش
        for aid in AUTHORIZED_USERS:
            try:
                buttons = [
                    [InlineKeyboardButton("🟦 دعوة فريق GO للنقاش", callback_data=f"team_main_{aid}")],
...
async def handle_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # نتأكد أن الزر فعلاً يخص هذا المستخدم
    ...
    await query.answer()
    # حذف رسالة الزر القديم
    try:
        await context.bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)
    except:
        pass

    # أزرار التقييم + زر الرجوع للقائمة الرئيسية
    keyboard = [
...
async def save_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ✅ رسالة شكر للمستخدم (اسم المستخدم بخط نحيف ورمادي كما هو بأسلوبك)
        thank_you_message = (
            f"🟦 شكراً لتقييمك،\n"
            f"`{user_name}`\n\n"
