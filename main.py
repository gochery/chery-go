...
def _update_go_stats_sync():
    """
    ✅ تحديث عدّاد استخدام GO مباشرة داخل شيت bot_stats
    - تقرأ الشيت
    - تزيد total_go_uses بواحد
    - تكتب التحديث في bot_data.xlsx
    """
    try:
        try:
        ...
        mask = stats_df["key"] == "total_go_uses"
        if mask.any():
            stats_df.loc[mask, "value"] = (
                stats_df.loc[mask, "value"].fillna(0).astype(int) + 1
            )
        else:
            stats_df = pd.concat(
        ...
        logging.info("[GO STATS] ✅ تم تحديث عداد GO في bot_stats مباشرة")
...
async def update_go_stats_async():
    """
    ✅ غلاف async:
    - يشغّل _update_go_stats_sync في ثريد منفصل
    - حتى لا يبطّئ ظهور GO أو شاشة الترحيب
    """
    try:
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, _update_go_stats_sync)
    except Exception as e:
        logging.error(f"[GO STATS] ❌ فشل تشغيل تحديث GO في الخلفية: {e}")
