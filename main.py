...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ------------------------------------------------------------------------
    # فرع الخاص بعد تعديل منطق إنهاء الجلسة
    # ------------------------------------------------------------------------
    context.user_data.setdefault(user_id, {})
    ...
    attempts = context.user_data[user_id].get("attempts_left", 3)
    if attempts >= 3:
        # إنهاء الجلسة السابقة تلقائيًا عند استنفاد المحاولات
        context.user_data[user_id]["session_valid"] = False
        context.user_data[user_id]["attempts_left"] = 0
        logging.info(f"[SESSION END] انتهت جلسة الدعم للمستخدم {user_name} بعد 3 محاولات")

    # إعادة ضبط الجلسة الحالية لتكون صالحة للخاص
    if not context.user_data[user_id].get("session_valid"):
        context.user_data[user_id]["session_valid"] = True
        context.user_data[user_id]["attempts_left"] = 3
    ...
    msg1 = await update.message.reply_text(
        ...
        "💡 *تم نقلك الآن لبداية جلسة استعلام تفاعلية…*\n\n"
