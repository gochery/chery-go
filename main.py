...
async def handle_suggestion_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # prefix = reply أو replyready أو sendreply
    ...
    # ===== استخراج user_id و suggestion_id بشكل موحد حسب نوع الزر =====
    if prefix == "sendreply":
        # sendreply_keyX_userid_suggestionid
        if len(data_parts) < 4:
            await query.answer("❌ بيانات غير صحيحة للرد الجاهز.", show_alert=True)
            return
        ready_key = data_parts[1]
    ...
    if prefix == "reply":
        ...
        # قائمة الردود الجاهزة (replyready أو sendreply)
        keyboard = [
