...
BRAND_CONTACTS = {
    ...
    "جيتور": {
        "company": "شركة التوريدات الوطنية للسيارات",
        "phone": "920051222",
    },
...
async def save_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # منع اي احد غير صاحب الجلسة من التقييم
    if query.from_user.id != user_id:
        requester = await context.bot.get_chat(user_id)
        await query.answer(
    ...
    try:
        ...
        # نحاول قراءة شيت ratings فقط
        try:
            df_existing = pd.read_excel(ratings_file, sheet_name="ratings")
        except FileNotFoundError:
            # ملف الإكسل كله غير موجود
            df_existing = pd.DataFrame(columns=list(rating_entry.keys()))
        except ValueError:
            # الملف موجود لكن شيت ratings غير موجود
            df_existing = pd.DataFrame(columns=list(rating_entry.keys()))
        except Exception as e:
            logging.error(f"[RATING] فشل قراءة شيت ratings من bot_data.xlsx: {e}")
            df_existing = pd.DataFrame(columns=list(rating_entry.keys()))

        df_combined = pd.concat(
            [df_existing, pd.DataFrame([rating_entry])],
            ignore_index=True
        )

        # نكتب فقط شيت ratings ولا نلمس الباقي
        with pd.ExcelWriter(
            ratings_file,
            engine="openpyxl",
            mode="a",
        ...
        # هنا يبان السبب الحقيقي في اللوق
